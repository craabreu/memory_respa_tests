...
//			char buf[BUF_SIZE];
	...
	if(g_bAdvanced2) {
		...
		if(_windowFunction == 1) {
			...
			_windowFunctionParameter = AskUnsignedInteger("    Parameter \"a\" (in time steps): [%d] ", m_iFinalDepth / 4, m_iFinalDepth / 4);
		} else if(_windowFunction == 3) {
			_windowFunctionParameter = AskUnsignedInteger("    Parameter \"a\" (in time steps): [%d] ", m_iFinalDepth / 2, m_iFinalDepth / 2);
...
void CPowerObservation::finalize() {
	...
	double step, possibleRange, integral;
	CFFT *fft;
...
_beginsplit:
	...
	if (_saveACF) {
		...
		integral = 0;
		fprintf(acfFile, "#Correlation Depth [fs];  Velocity ACF [pm^2/ps^2];  Integral [pm^2/ps]\n");
		for(i = 0; i < _correlationDepth; i++) {
	...
	if (g_bPower) {
		...
		if(m_iShowMol == -1)
			mprintf("    Assuming %dn = %d degrees of freedom, the average temperature is %.3f K.\n", cc, cc * g_iGesAtomCount, integral / (double(cc) * g_iGesAtomCount));
		else
			mprintf("    Assuming %dn = %d degrees of freedom, the average temperature is %.3f K.\n", cc, cc * _atoms->m_iAtomGes, integral / (double(cc) * _atoms->m_iAtomGes));
...
void processDipoleRestart(CTimeStep *ts) {
	...
	for (i = 0; i < g_oaSingleMolecules.GetSize(); i++) {
		...
		for (j = 0; j < 3; j++) {
			...
			fwrite(&val, sizeof(double), 1, g_dipoleRestartFile);
		}
	}
...
void processMagneticDipoleRestart(CTimeStep *ts) {
	...
	for (i = 0; i < g_oaSingleMolecules.GetSize(); i++) {
		...
		for (j = 0; j < 3; j++) {
			...
			fwrite(&val, sizeof(double), 1, g_magneticDipoleRestartFile);
		}
	}
