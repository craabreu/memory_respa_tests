 ...
 */
{
	...
	if( !printflags )
	{
		if (iflag == 2)
		{
			if (g_fLSpecEvolveBuf != NULL)
			{
				int z;
				for (z=0;z<g_iFitDegree;z++)
					g_fLSpecEvolveBuf[g_iLMPrintTmp*g_iFitDegree+z] = par[z];
			}
			g_iLMPrintTmp++;
			if (fmod(g_iLMPrintTmp,g_iLMMaxIter/60.0) < 1.0)
			{
//				g_iLMPrintTmp = 0;
				if (!g_bLMFitSilent)
				{
					mprintf(WHITE,"#");
/*					int z2;
					double t, t2, sse, sst, ya, r;

					ya = 0;
					for (z2=0;z2<m_dat;z2++)
						ya += ((lmcurve_data_struct*)data)->y[z2];
					ya /= m_dat;
					...
					for (z2=0;z2<m_dat;z2++)
					{
						...
						t2 += pow( fabs(((lmcurve_data_struct*)data)->y[z2] - t), 2);
						sse += (((lmcurve_data_struct*)data)->y[z2]-t)*(((lmcurve_data_struct*)data)->y[z2]-t);
						sst += (((lmcurve_data_struct*)data)->y[z2]-ya)*(((lmcurve_data_struct*)data)->y[z2]-ya);
					}
					r = sqrt(1.0 - sse/sst);

					mprintf("      %4d: Fit RMS Error = %15.8f;  R = %10.8f;  sse=%G;  sst=%G;  sse/sst=%G.\n",g_iLMPrintTmp,t2,r,sse,sst,sse/sst);
					for (z2=0;z2<g_iFitDegree;z2++)
					{
						if ((z2%10) == 0)
							mprintf("\n      ");
						mprintf("%10G, ",par[z2]);
	...
	if( printflags & 1 ){
		...
		if (iflag == 2) {
			mprintf("trying step in gradient direction  ");
		} else if (iflag == 1) {
			mprintf("determining gradient (iteration %2d)", iter);
		} else if (iflag == 0) {
			mprintf("starting minimization              ");
		} else if (iflag == -1) {
			mprintf("terminated after %3d evaluations   ", nfev);
		}
	}
...
/*** lmdif: the outer loop. ***/
    ...
    do {
#ifdef LMFIT_DEBUG_MESSAGES
	mprintf("lmdif/ outer loop iter=%d nfev=%d fnorm=%.10e\n",
	       iter, *nfev, fnorm);
#endif
...
//		mprintf("\n#Iter%3d: replacing %.20G by %.20G.",j,temp,x[j]);
	    *info = 0;
...
/*** the inner loop. ***/
	do {
#ifdef LMFIT_DEBUG_MESSAGES
	    mprintf("lmdif/ inner loop iter=%d nfev=%d\n", iter, *nfev);
#endif
...
#ifdef LMFIT_DEBUG_MESSAGES
	    mprintf("lmdif/ pnorm %.10e  fnorm1 %.10e  fnorm %.10e"
		   " delta=%.10e par=%.10e\n",
...
#ifdef LMFIT_DEBUG_MESSAGES
	    mprintf("lmdif/ actred=%.10e prered=%.10e ratio=%.10e"
...
/*** inner: test for successful iteration. ***/
		...
		mprintf("ATTN: iteration considered unsuccessful\n");
	    }
#endif
...
//	if ((fabs(r[j * ldr + j]) < 1e-100) && (nsing == n))
	    nsing = j;
	if (nsing < n)
	    aux[j] = 0;
    }
//#ifdef LMFIT_DEBUG_MESSAGES
    ...
    for (j = nsing - 1; j >= 0; j--) {
#ifdef LMFIT_MYDEBUG
	mprintf("\n$ A aux[%d]=%G, r[%d + %d*%d]=%G.",j,aux[j],j,ldr,j,r[j + ldr * j]);
#endif
...
/*** lmpar: initialize the iteration counter, evaluate the function at the
    ...
    for (j = 0; j < n; j++)
	{
//	mprintf("\n$ diag[%d]=%G, x[%d]=%G.",j,diag[j],j,x[j]);
	xdi[j] = diag[j] * x[j];
	}
    dxnorm = lm_enorm(n, xdi);
#ifdef LMFIT_MYDEBUG
	mprintf("\n$ dxnorm=%G, delta=%G.",dxnorm,delta);
#endif
    fp = dxnorm - delta;
    if (fp <= p1 * delta) {
#ifdef LMFIT_DEBUG_MESSAGES
	mprintf("lmpar/ terminate (fp<p1*delta)\n");
#endif
	*par = 0;
	return;
    }
...
/*** lmpar: if the jacobian is not rank deficient, the newton
	...
	temp = lm_enorm(n, aux);
#ifdef LMFIT_MYDEBUG
	mprintf("\n$ fp=%G, delta=%G, temp=%G, sum=%G, dxnorm=%G.",fp,delta,temp,sum,dxnorm);
#endif
...
/*** lmpar: if the input par lies outside of the interval (parl,paru),
	...
	*par = gnorm / dxnorm;
#ifdef LMFIT_DEBUG_MESSAGES
    mprintf("lmpar/ parl %.4e  par %.4e  paru %.4e\n", parl, *par, paru);
#endif
...
#ifdef LMFIT_MYDEBUG
	mprintf("\n$ %G = MAX( %G, %G + %G ).",MAX(parl, *par + parc),parl,*par,parc);
#endif
...
// Change by M. Brehm, July 3rd, 2013 
	if (fabs(ajnorm) < 1e-200) {
	    rdiag[j] = 0;
	    continue;
	}
