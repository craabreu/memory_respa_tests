...
extern double g_fBoxX;
extern double g_fBoxY;
extern double g_fBoxZ;
...
public:
	...
	void AddToBin(const CxDVector3 &vec)
	{
		AddToBin(vec[0],vec[1],vec[2]);
	}


	void AddToBin_Sphere(const CxDVector3 &vec, double r)
	{
		AddToBin_Sphere(vec[0],vec[1],vec[2],r);
	}


	void AddToBin_SphereWrap(const CxDVector3 &vec, double r)
	{
		AddToBin_SphereWrap(vec[0],vec[1],vec[2],r);
	}


	void AddToBin_Single(double x, double y, double z, T val)
	{
		int ix, iy, iz;
	...
	T GetBinValue_Single(double x, double y, double z) const
	{
		int ix, iy, iz;
	...
	void SetBinValue_Single(double x, double y, double z, T val)
	{
		int ix, iy, iz;
...
/******************************************************************************************/
	...
	T GetValue(const CxDVector3 &vec)
	{
				...
				for (y=0;y<m_iRes[1]+2;y++)
				{
					for (x=0;x<m_iRes[0]+2;x++)
					{
						if ((z == 0) || (z == m_iRes[2]+1) || (y == 0) || (y == m_iRes[1]+1) || (x == 0) || (x == m_iRes[0]+1))
							...
							ff = (float)m_pBin[(z-1)*m_iRes[0]*m_iRes[1] + (y-1)*m_iRes[0] + x-1];
						fwrite(&ff,4,1,a);
					}
				}
			}
...
//		strcat(buf,suffix);
		...
		if (sdf)
		{
			mfprintf(a,"1 %f %f %f\n",(m_fMinVal[0]+0.5*(m_fMaxVal[0]-m_fMinVal[0])/m_iRes[0])/100.0/0.529177249,(m_fMinVal[1]+0.5*(m_fMaxVal[1]-m_fMinVal[1])/m_iRes[1])/100.0/0.529177,(m_fMinVal[2]+0.5*(m_fMaxVal[2]-m_fMinVal[2])/m_iRes[2])/100.0/0.529177);
	...
	void ReadCube(FILE *a, bool verbose, bool create)
	{
		...
		*p = 0;
		...
		if (feof(a))
		{
			eprintf("C3DF::ReadCube(): Error: Unexpected end of cube file stream.\n");
			abort();
		}
		...
		} else
		{
			...
			{
				eprintf("C3DF::ReadCube(): Error: Cube file dimension mismatch (%d-%d, %d-%d, %d-%d).\n",rx,m_iRes[0],ry,m_iRes[1],rz,m_iRes[2]);
				abort();
			}
			for (z=0;z<m_iRes[0]*m_iRes[1]*m_iRes[2];z++)
				m_pBin[z] = 0;
	...
	// 			break;
		...
		if (feof(a))
		{
			eprintf("C3DF::ReadCubeData(): Error: Unexpected end of cube file stream.\n");
			abort();
		}
	...
	void ReadCubeData(CBQBCubeFrame *cfr, bool verbose)
	{
		...
		{
	_next:
			m_pBin[iz*m_iResXY+iy*m_iRes[0]+ix] = cfr->m_faBin[iz+iy*m_iRes[2]+ix*m_iRes[1]*m_iRes[2]];

			if (verbose)
				su += cfr->m_faBin[iz*m_iResXY+iy*m_iRes[0]+ix];
