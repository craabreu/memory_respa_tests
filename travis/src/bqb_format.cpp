...
bool CBQBFile::ReadFrame() {
	...
	if (m_bListFile) {
		...
		if (m_oaBQBList.size() == 0) {
			if (m_IF.IsPL(BQB_PL_VERBOSE))
				m_IF.printf("    LIST: Error: No entrys in list.\n");
			m_bEOF = true;
			return false;
		}
...
_again:
		...
		if (memcmp(uci,"BQ",2) != 0) {
			m_IF.eprintf("CBQBFile::ReadFrame(): Error: Frame does not start with \"BQ\".\n");
			abort();
		}
		...
		if ((ty < 2) || (ty > 13)) {
			m_IF.eprintf("CBQBFile::ReadFrame(): Error: Frame type %d is not a valid short frame.\n",ty);
			abort();
		}
		...
		if (!ReadArrayFromFile(m_pFile,m_pShortFrame->m_iaPayload,len)) {
			m_IF.eprintf("CBQBFile::ReadFrame(): Error: Unexpected end of file while reading frame payload (%u bytes).\n",len);
			abort();
		}
		...
		if (crc != m_pShortFrame->m_iCRC32) {
			m_IF.eprintf("CBQBFile::ReadFrame(): Error: CRC check failed for frame payload (computed %08lX, read %08X).\n",m_pShortFrame->m_iCRC32,crc);
			abort();
		}
...
CBQBEngine* CBQBFile::GetEngine() {
	...
	if (m_bListFile) {
		...
		if (m_oaBQBList.size() == 0) {
			m_IF.eprintf("CBQBFile::GetEngine(): Error: No entrys in list.\n");
			abort();
		}
	...
	if (m_pIndex->m_iaFrameLengths.size() != 0) {
		...
		while (ReadFrame()) {
			m_IF.printf("    Frame %6d,  type %2d v%d (%s),  ID %6d,  payload size %8lu bytes.\n",
				...
				m_pShortFrame->m_iID,
				m_pShortFrame->m_iaPayload.size()
			);
			if (b) {
...
_abs:
	...
	if (!b) {
		...
		m_pIndex = new CBQBIndex(m_IF);
		for (z=0;z<(int)m_oaBQBList.size();z++) {
			m_pIndex->m_iaFrameLengths.insert(m_pIndex->m_iaFrameLengths.end(),m_oaBQBList[z]->m_pIndex->m_iaFrameLengths.begin(),m_oaBQBList[z]->m_pIndex->m_iaFrameLengths.end());
			m_pIndex->m_iaFrameTypes.insert(m_pIndex->m_iaFrameTypes.end(),m_oaBQBList[z]->m_pIndex->m_iaFrameTypes.begin(),m_oaBQBList[z]->m_pIndex->m_iaFrameTypes.end());
		}
	} else {
		if (m_IF.IsPL(BQB_PL_VERBOSE))
	...
	if (m_IF.IsPL(BQB_PL_VERBOSE))
		m_IF.printf("LIST: Added %lu entrys to list.\n",m_oaBQBList.size());
...
bool CBQBFile::SeekFrame(int i) {
	...
	if (m_bListFile) {
		...
		if (m_oaBQBList.size() == 0) {
			if (m_IF.IsPL(BQB_PL_VERBOSE))
				m_IF.eprintf("CBQBFile::SeekFrame(): Error: No entrys in list.\n");
			return false;
		}
