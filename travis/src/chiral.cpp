...
//		char buf[BUF_SIZE];
		...
		if(ma == NULL) {
			eprintf("CChiralObservation::CChiralObservation(): Internal error.\n");
			abort();
		}
		...
		if(ma->m_oaBonds.GetSize() == 4) {
			...
			for(i = 0; i < 4; i++) {
				if(i > 0)
					mprintf(", ");
				mprintf("%s%d", (const char*)((CAtom *)g_oaAtoms[sm->m_baAtomIndex[((CMolAtom *)ma->m_oaBonds[i])->m_iType]])->m_sName, ((CMolAtom *)ma->m_oaBonds[i])->m_iNumber+1);
			}
			mprintf(".\n");
			break;
		}
	...
	for(i = 0; i < 4; i++) {
		...
		if(i > 0)
			mprintf(" > ");
		mprintf("%s%d", (const char*)((CAtom *)g_oaAtoms[((CSingleMolecule *)g_oaSingleMolecules[((CMolecule *)g_oaMolecules[m_iShowMol])->m_laSingleMolIndex[0]])->m_baAtomIndex[((CMolAtom *)((CMolAtom *)_molAtoms[0])->m_oaBonds[a])->m_iType]])->m_sName, ((CMolAtom *)((CMolAtom *)_molAtoms[0])->m_oaBonds[a])->m_iNumber+1);
	}
	mprintf(".\n");
...
/*#ifdef TARGET_LINUX
				...
				buf2.sprintf("%s%d=%d", (const char*)((CAtom *)g_oaAtoms[((CSingleMolecule *)g_oaSingleMolecules[((CMolecule *)g_oaMolecules[m_iShowMol])->m_laSingleMolIndex[0]])->m_baAtomIndex[((CMolAtom *)((CMolAtom *)_molAtoms[0])->m_oaBonds[a])->m_iType]])->m_sName, ((CMolAtom *)((CMolAtom *)_molAtoms[0])->m_oaBonds[a])->m_iNumber+1, i+1);
				buf.strcat(buf2);
...
CChiralObservation::~CChiralObservation() {
	...
	for(i = 0; i < m_iShowMolCount; i++) {
		...
		delete (CxDoubleArray *)_timedev[i];
	}
}
...
void CChiralObservation::process(CTimeStep *ts) {
	...
	for(i = 0; i < m_iShowMolCount; i++) {
		...
		double dih = Dihedral(vec1, vec2, vec3, false);
		((CxDoubleArray *)_timedev[i])->Add(dih);
	}
}
...
#endif*/
	name.sprintf("chiral_%s_%s%d.dat", ((CMolecule *)g_oaMolecules[m_iShowMol])->m_sName, (const char*)((CAtom *)g_oaAtoms[((CSingleMolecule *)g_oaSingleMolecules[((CMolecule *)g_oaMolecules[m_iShowMol])->m_laSingleMolIndex[0]])->m_baAtomIndex[((CMolAtom *)_molAtoms[0])->m_iType]])->m_sName, ((CMolAtom *)_molAtoms[0])->m_iNumber+1);

	mprintf("    Writing chirality data to %s...\n", (const char*)name);
	FILE *chiralFile = OpenFileWrite(name, false);
	
	for(i = 0; i < ((CxDoubleArray *)_timedev[0])->GetSize(); i++) {
		fprintf(chiralFile, "%d", i+1);
		for(j = 0; j < m_iShowMolCount; j++) {
			if(i > 0) {
				if((((CxDoubleArray *)_timedev[j])->GetAt(i)) > 0.0 && (((CxDoubleArray *)_timedev[j])->GetAt(i-1) <= 0.0))
			...
			fprintf(chiralFile, " %s", ((CxDoubleArray *)_timedev[j])->GetAt(i) > 0.0 ? "R" : "S");
		}
		fprintf(chiralFile, "\n");
	}
	fclose(chiralFile);
...
int CChiralObservation::swapAtom(int offset) {
	...
	if(a != -1) {
		bool lastStepR = ((CxDoubleArray *)_timedev[a])->GetAt(((CxDoubleArray *)_timedev[a])->GetSize()-1) > 0.0;
...
void processChiral(CTimeStep *ts) {
	...
	if(g_fixTraj) {
		...
		for(i = 0; i < fixedOrder.GetSize(); i++) {
			fprintf(g_fixedTrajFile, "%4s %14.10f %14.10f %14.10f\n", (const char*)((CAtom *)g_oaAtoms[g_waAtomRealElement[fixedOrder[i]]])->m_sName, ts->m_vaCoords_Original[fixedOrder[i]][0] / 100.0, ts->m_vaCoords_Original[fixedOrder[i]][1] / 100.0, ts->m_vaCoords_Original[fixedOrder[i]][2] / 100.0);
		}
	}
}
