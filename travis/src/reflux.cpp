...
void CReFluxObservation::initialize()
{
	...
	while ( m_iTrunc > (int)(m_iDepth/10) )
	{
		...
		m_iTrunc = AskUnsignedInteger("    Enter the truncation for the fitting procedure (in time steps): [%d] ", m_iTrunc, m_iTrunc);
...
void CReFluxObservation::SortDist()
{
	...
	while ( true )
	{
		mprintf("\n    Which couples should be ignored (comma seperated numbers) [none]?");
		inpprintf("! Which couples should be ignored (comma seperated numbers) [none]?\n");
		myget(&buf);
		if ( strlen(buf) != 0 ) 
	...
	while ( true )
	{
		mprintf("\n    Which criteria should be paired (comma seperated numbers) [end]?");
		inpprintf("! Which criteria should be paired (comma seperated numbers) [end]?\n");
		myget(&buf);
		if ( strlen(buf) == 0 )
			break;
		if ( !GetArray(buf, &temp, ((CReFluxCond*)m_oaCond[0])->m_oaDistConds.GetSize()) )
...
void CReFluxObservation::SortAngle()
{
	...
	while ( true )
	{
		mprintf("\n    Which definitions should be ignored (comma seperated numbers) [none]?");
		inpprintf("! Which definitions should be ignored (comma seperated numbers) [none]?\n");
		myget(&buf);
		if ( strlen(buf) != 0 ) 
...
void CReFluxObservation::SortFinal()
{
	...
	while ( true )
	{
		mprintf("\n    Which criteria should be discarded (comma seperated numbers) [none]?");
		inpprintf("! Which criteria should be discarded (comma seperated numbers) [none]?\n");
		myget(&buf);
		if ( strlen(buf) != 0 ) 
