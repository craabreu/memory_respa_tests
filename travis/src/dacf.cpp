...
//	sprintf(buf,"%s_%s",((CMolecule*)g_oaMolecules[m_iFirstMol])->m_sName,((CMolecule*)g_oaMolecules[m_iSecondMol])->m_sName);
	...
	if (g_bDACF)
	{
		...
		if (m_bFitDACF)
		{
			...
			m_iFitDegreeMax = AskUnsignedInteger("    How many exponential terms to use at most: [3] ",3);
			mprintf("\n");
		}
			m_bLifetimeSpectrum = false;
	}
...
_ca1:	mprintf("    Enter atoms from 1st molecule [center of mass]: ");
		...
		} else
		{
			...
			{
				eprintf("CDACF::Parse(): Internal error.\n");
			...
			switch(ti)
			{
				...
				case 3:
					for (z=0;z<m_pCenterAtoms1->m_baAtomType.GetSize();z++)
						for (z2=0;z2<((CxIntArray*)m_pCenterAtoms1->m_oaAtoms[z])->GetSize();z2++)
						{
							tf = AskFloat("    Enter weight for %s%d: [1.0] ",1.0,(const char*)((CAtom*)g_oaAtoms[m_pCenterAtoms1->m_baRealAtomType[z]])->m_sName,((CxIntArray*)m_pCenterAtoms1->m_oaAtoms[z])->GetAt(z2)+1);
							m_faWeight1.Add(tf);
						}
					break;
				default:
			...
			switch(ti)
			{
				...
				case 3:
					for (z=0;z<m_pCenterAtoms2->m_baAtomType.GetSize();z++)
						for (z2=0;z2<((CxIntArray*)m_pCenterAtoms2->m_oaAtoms[z])->GetSize();z2++)
						{
							tf = AskFloat("    Enter weight for %s%d: [1.0] ",1.0,(const char*)((CAtom*)g_oaAtoms[m_pCenterAtoms2->m_baRealAtomType[z]])->m_sName,((CxIntArray*)m_pCenterAtoms2->m_oaAtoms[z])->GetAt(z2)+1);
							m_faWeight2.Add(tf);
						}
					break;
				default:
...
_mwagain:	
		...
		switch(ti)
		{
			...
			case 2:
				...
				break;
			case 3:
				tf = AskFloat("    Enter weight for 1st molecule: [1.0] ",1.0);
				tf2 = AskFloat("    Enter weight for 2nd molecule: [1.0] ",1.0);
				m_fWeightMol1 = tf / (tf+tf2);
				break;
			default:
				goto _mwagain;
		}
		mprintf("    Weight Mol.1 : Mol.2 is %.3f : %.3f.\n\n",m_fWeightMol1,1.0-m_fWeightMol1);
		m_fLargestDisplacement = AskFloat("    Enter largest displacement in pm [1000.0]: ",1000.0);
		m_iDisplacementRes = AskUnsignedInteger("    Enter displacement resolution [100]: ",100);
		m_fMaxVel = AskFloat("    Enter maximum dimer center velocity in pm/ps (0 means no max. vel.): [10000.0] ",10000.0);
		if (m_fMaxVel != 0)
		...
		} else
		{
			if (g_iTrajSteps != -1)
				m_fLargestLifetime = AskFloat("    Enter largest lifetime in ps [%.3f]: ",g_iTrajSteps*0.75*g_fTimestepLength/1000.0,g_iTrajSteps*0.75*g_fTimestepLength/1000.0);
					else m_fLargestLifetime = AskFloat("    Enter largest lifetime in ps [5.0]: ",5.0);
			m_iLifetimeRes = AskUnsignedInteger("    Enter lifetime binning resolution [%d]: ",int(m_fLargestLifetime*1000.0/g_fTimestepLength),int(m_fLargestLifetime*1000.0/g_fTimestepLength));
		}
...
void CDACFSub::Parse()
{
	...
	m_bIntermittend = (AskRangeInteger("    Calculate this function as continuous (0) or intermittent (1) type? [1] ",0,1,1) == 1);

	if (m_bIntermittend) {
		m_bNewMode = true;
/*		if (!g_bInterWarning) {
			g_bInterWarning = true;
			mprintf("\n");
			mprintf(RED,"Warning: ");
			mprintf("The TRAVIS implementation of intermittent ACFs was flawed; this bug was fixed\n");
...
CxDVector3 CDACF::CalcCenter(CxDVec3Array *v, int i1, int i2)
{
	CxDVector3 v1, v2;
	int z, z2, c;
	...
	v1 = v1 * m_fWeightMol1 + v2 * (1.0 - m_fWeightMol1);
...
//		smfix = (CSingleMolecule*)g_oaSingleMolecules[((CMolecule*)g_oaMolecules[m_iFirstMol])->m_laSingleMolIndex[z2]];
		...
		//		if ((!dacfsub->m_bIntTravisStyle) && (*/ag->m_iEnd == (long)g_iSteps))
				...
				if (g_bDDisp || g_bPairMSD)
				{
					...
					if (g_bVerbose)
						mprintf("\n## %lu: (Int) Aggregate %d.%d lived from %lu to %ld, end at ( %.2f | %.2f | %.2f ), traveled %.2f pm.",g_iSteps,z2,j,ag->m_iFirstStart,ag->m_iEnd,ag->m_vEnd[0],ag->m_vEnd[1],ag->m_vEnd[2],(ag->m_vEnd-ag->m_vFirstStart).GetLength());
				}
				...
				if (ag->m_iEnd == -1)
				{
					...
					if (g_bDDisp || g_bPairMSD)
					{
						...
						if (m_fMaxVel != 0)
						{
							if (tf/tf2 > m_fMaxVel)
							{
								mprintf("\n    Dimer %d - %d lived from Step %lu to %lu and moved %.2f pm (%.1f pm/ps) - too fast.",z2+1,ag->m_iSingleMol2+1,ag->m_iStart,g_iSteps,tf,tf/tf2*1000.0);
								if (m_bRemoveMaxVel)
									goto _maxvel2;
							}
						}
	...
	for (z3=0;z3<((CMolecule*)g_oaMolecules[m_iSecondMol])->m_laSingleMolIndex.GetSize();z3++) // Alle anderen Molekuele durchgehen
	{
		...
		if (dacfsub->m_pCondition->Contains(z3))
		{
			...
			if (g_bDDisp || g_bPairMSD)
			{
				...
				if (g_bVerbose)
					mprintf("\n## %lu: Aggregate %d.%d spawned at ( %.2f | %.2f | %.2f ).",g_iSteps,rm,dacfsub->m_oaAggregates[rm].GetSize()-1,ag->m_vFirstStart[0],ag->m_vFirstStart[1],ag->m_vFirstStart[2]);
			}
...
/*	if (dacfsub->m_bIntermittend && !dacfsub->m_bNewMode)
	{
		for (z4=0;z4<dacfsub->m_oaAggregates[rm].GetSize();z4++)
		{
			ag = (CAggregate*)dacfsub->m_oaAggregates[rm][z4];
...
//							dacfsub->m_pDACF->AddToBin_Count(z5,ti-z5);
	...
	//			mprintf("Int: RM=%d, OM=%d, t=%d, FirstStart=%d, End=%d, MaxGap ueberschritten.\n",rm+1,ag->m_iSingleMol2+1,g_iSteps,ag->m_iFirstStart,ag->m_iEnd);
			...
			{ // Currently together
				...
				if (g_bVerbose)
				{
					tv = CalcCenter(&t->m_vaCoords_Unfolded,rm,ag->m_iSingleMol2);
					tf = (tv-ag->m_vFirstStart).GetLength();
					mprintf("\n## %lu: (Int) Aggregate %d.%d lives since %lu at ( %.2f | %.2f | %.2f ), traveled %.2f pm.",g_iSteps,rm,z4,ag->m_iFirstStart,tv[0],tv[1],tv[2],tf);
				}
			}
		} // END FOR Z4
	} else // IF NOT INTERMITTENT
		...
		for (z4=0;z4<dacfsub->m_oaAggregates[rm].GetSize();z4++) {
			...
			if ((!ag->m_bStillAlive) && (ag->m_iEnd == -1)) {
				...
				if (g_bDDisp || g_bPairMSD) {
					...
					if (g_bVerbose)
						mprintf("\n## %lu: Aggregate %d.%d died at ( %.2f | %.2f | %.2f ), traveled %.2f pm.",g_iSteps,rm,z4,ag->m_vEnd[0],ag->m_vEnd[1],ag->m_vEnd[2],tf);
					
					if (m_fMaxVel != 0) {
						if (tf/tf2 > m_fMaxVel) {
							mprintf("\nDimer %d - %d lived from Step %lu to %lu and moved %.2f pm (%.1f pm/ps) - too fast.",rm+1,ag->m_iSingleMol2+1,ag->m_iStart,g_iSteps,tf,tf/tf2*1000.0);
							if (m_bRemoveMaxVel)
								goto _maxvel;
						}
					}
		...
		_maxvel:
				...
				//		mprintf("%d*%d+%d=%d\n",rm,((CMolecule*)g_oaMolecules[m_iSecondMol])->m_laSingleMolIndex.GetSize(),ag->m_iSingleMol2,rm*((CMolecule*)g_oaMolecules[m_iSecondMol])->m_laSingleMolIndex.GetSize()+ag->m_iSingleMol2);
						...
						if (m_iDACFRes < (int)(ag->m_iEnd-ag->m_iStart+1)) {
							...
							ti = (int)(ag->m_iEnd-ag->m_iStart+1);
	...
	//			mprintf("Con: RM=%d, OM=%d, t=%d, FirstStart=%d, End=%d.\n",rm+1,ag->m_iSingleMol2+1,g_iSteps,ag->m_iFirstStart,ag->m_iEnd);
				...
				if (g_bVerbose) {
					tv = CalcCenter(&t->m_vaCoords_Unfolded,rm,ag->m_iSingleMol2);
					tf = (tv-ag->m_vStart).GetLength();
					mprintf("\n## %lu: Aggregate %d.%d lives at ( %.2f | %.2f | %.2f ), traveled %.2f pm.",g_iSteps,rm,z4,ag->m_vEnd[0],ag->m_vEnd[1],ag->m_vEnd[2],tf);
				}
			}
		} // END FOR Z4
//	} // END IF NOT INTERMITTEND
...
void CDACFSub::ProcessPair(const CxIntArray &intervals)
{
	int z, z2, z3, i;

	for (z=0;z<intervals.GetSize();z+=2) {
		i = 0;
		for (z2=intervals[z+1]-intervals[z]+1;z2>0;z2--)
			m_pDACF->m_pBin[i++] += z2;
	}

	for (z=0;z<intervals.GetSize();z+=2) {
		for (z2=z+2;z2<intervals.GetSize();z2+=2) {
			i = intervals[z2] - intervals[z+1];
			if ((intervals[z+1]-intervals[z]) >= (intervals[z2+1]-intervals[z2])) {
				for (z3=1;z3<=intervals[z2+1]-intervals[z2]+1;z3++)
					m_pDACF->m_pBin[i++] += z3;
				for (z3=0;z3<(intervals[z+1]-intervals[z])-(intervals[z2+1]-intervals[z2]);z3++)
					m_pDACF->m_pBin[i++] += intervals[z2+1]-intervals[z2]+1;
				for (z3=intervals[z2+1]-intervals[z2];z3>0;z3--)
					m_pDACF->m_pBin[i++] += z3;
			} else {
				for (z3=1;z3<=intervals[z+1]-intervals[z]+1;z3++)
					m_pDACF->m_pBin[i++] += z3;
				for (z3=0;z3<(intervals[z2+1]-intervals[z2])-(intervals[z+1]-intervals[z]);z3++)
					m_pDACF->m_pBin[i++] += intervals[z+1]-intervals[z]+1;
				for (z3=intervals[z+1]-intervals[z];z3>0;z3--)
					m_pDACF->m_pBin[i++] += z3;
			}
		}
	}
