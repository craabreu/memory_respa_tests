 ...
 * \param[in,out] up */
inline bool voronoicell_base::search_for_outside_edge(voronoicell_neighbor &vc,int &upparm) {
	int i,lp,lw,*j = (ds2),*stackp2 = (ds2);
	double l;
	*(stackp2++)=upparm;
	while(j<stackp2) {
		upparm=*(j++);
		for(i=0;i<nu[upparm];i++) {
			lp=ed[upparm][i];
			lw=m_test(lp,l);
			if(lw==-1) return true;
...
inline void voronoicell_base::add_to_stack(voronoicell_neighbor &vc,int lp,int *&stackp2) {
	UNUSED(vc); /* Suppress "unused parameter" warning */
...
bool voronoicell_base::nplane(voronoicell_neighbor &vc,double x,double y,double z,double rsq,int p_id) {
	...
	try {
		if(uw==1) {
			...
			while(lw==1) {
				if(++count>=p) {
					eprintf("voronoicell_base::nplane(): Internal error.\n");
					throw true;
				}
				u=l;up=lp;
				for(us=0;us<ls;us++) {
