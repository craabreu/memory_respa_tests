...
// 		m_vaCoords.Add(CxDVector3(fx*0.529177249f*100.0f,fy*0.529177249f*100.0f,fz*0.529177249f*100.0f));
...
bool CTimeStep::ReadLAMMPS(FILE *a, bool needinfo)
{
	...
	while (true)
	{
		...
		{
	//		mprintf("\n### \"%s\" ###\n",(const char*)buf);
			if (strstr(buf,"ITEM: ATOMS element xu yu zu fx fy fz") != NULL)
			{
				g_bLAMMPSForce = true;
			} else if (strstr(buf,"ITEM: ATOMS element xu yu zu") != NULL)
			{
		...
		//			}
				...
				*p = 0;
				...
				m_vaCoords[i] = CxDVector3(x,y,z);

				if (g_bKeepOriginalCoords)
					m_vaCoords_Original[i] = CxDVector3(x,y,z);
				...
				if (g_bLAMMPSForce) {
					...
					for (z2=0;z2<3;z2++) {
						...
						if ((z2 < 2) && (*p == 0))
						{
							eprintf("\nCTimeStep::ReadLAMMPS(): %d: Incomplete line while reading forces (%dth column missing) \"%s\"",i+1,z2+5,obuf);
							return false;
						}
						if (*p != 0)
							*p = 0;
						//z = (double)atof(q) * 100.0;
						#ifdef FAST_ATOF
							m_vaForces[i][z2] = fast_atof(q);
						#else
							m_vaForces[i][z2] = atof(q);
						#endif
					}
				}
