...
_okcfile:;
		...
		if (gc_bCube) {
			...
			if (gc_iCubeOptimize == 3)
				gc_pBQBInterface->printf("To achieve highest compression ratio (at the cost of compression time), add \"-csplit 14 -citer 40\".\n\n");
...
/*		if (!bf.CheckIntegrity(gc_sInFile,gc_bCheckVerbose)) {
...
_end:
	...
	BQBDestroyInterface( gc_pBQBInterface );
