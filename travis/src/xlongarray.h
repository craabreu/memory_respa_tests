...
public:
	CxLongArray();
	~CxLongArray();
	CxLongArray(const CxLongArray &o);
	explicit CxLongArray(const char *name);
	...
	long &GetAt(unsigned long i)
	{
		...
		{
			if (m_sName != NULL)
				eprintf("CxLongArray \"%s\" Boundary Error (%lu/%lu).\n",m_sName,i,m_iSize);
					else eprintf("CxLongArray Boundary Error (%lu/%lu).\n",i,m_iSize);
			abort();
		}
#endif
#ifdef DEBUG_CLONGARRAY
	...
	long GetAt(unsigned long i) const
	{
		BXIN;
#ifdef DEBUG_CLONGARRAY
		mprintf("@ CxLongArray::GetAt(int): %d...",i);
	...
	long operator [] (unsigned long i) const
	{
#ifdef DEBUG_CLONGARRAY
		mprintf("@ CxLongArray::operator [] (int): %d\n",i);
#endif
