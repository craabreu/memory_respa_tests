	...
	if (optimize != 0) {
		...
		if (m_iOptIncludeFirst == -1) {
			if ((m_pEngine->m_pReadCacheCube->GetCachedSteps() < optsteps) || (optsteps < 10)) {
				if (m_IF.IsPL(BQB_PL_STANDARD))
					m_IF.printf("        Low step count, switching on -optstart.\n\n");
				m_iOptIncludeFirst = 1;
			} else {
				if (m_IF.IsPL(BQB_PL_STANDARD))
					m_IF.printf("        Large step count, switching off -optstart.\n\n");
...
_skerr:
			if (err) {
				if (k != 0) {
					m_IF.eprintf("Errors occured. Compressing frame again with history zero.\n");
					err = false;
					k = 0;
					goto _again;
				} else {
					m_IF.eprintf("Errors occured despite of zero history. Aborting.\n");
					goto _end;
				}
			}
...
_end:
	...
	if (err) {
		...
		m_IF.printf("%10.3f MiB (%12s Bytes) alphabet data.\n",
			double(stat.m_oStat.m_lAlphabet)/1024.0/1024.0/8.0,
			(stat.m_oStat.m_lAlphabet>>3).string());
		m_IF.printf("%10.3f MiB (%12s Bytes) Huffman tables.\n",
			double(stat.m_oStat.m_lHuffmanTables)/1024.0/1024.0/8.0,
			(stat.m_oStat.m_lHuffmanTables>>3).string());
		m_IF.printf("%10.3f MiB (%12s Bytes) table switching.\n",
			double(stat.m_oStat.m_lTableSwitch)/1024.0/1024.0/8.0,
			(stat.m_oStat.m_lTableSwitch>>3).string());
		m_IF.printf("%10.3f MiB (%12s Bytes) payload data.\n",
			double(stat.m_oStat.m_lHuffmanData)/1024.0/1024.0/8.0,
			(stat.m_oStat.m_lHuffmanData>>3).string());
		m_IF.printf("%10.3f MiB (%12s Bytes) in total.\n\n",
			double(stat.m_oStat.m_lOverhead+stat.m_oStat.m_lAlphabet+stat.m_oStat.m_lHuffmanTables+stat.m_oStat.m_lTableSwitch+stat.m_oStat.m_lHuffmanData)/1024.0/1024.0/8.0,
...
double CBQBDriver::CompressCubeBenchmark(
	...
	while (true) {
		...
		if ((i+1 > parm->GetExtraTRange()) || (m_iOptIncludeFirst > 0)) {
...
_end:
	...
	if (err) {
		...
		m_IF.printf("%10.3f MiB (%12s Bytes) overhead.\n",
			double(stat.m_oStat.m_lOverhead)/1024.0/1024.0/8.0,(stat.m_oStat.m_lOverhead>>3).string());
		m_IF.printf("%10.3f MiB (%12s Bytes) alphabet data.\n",
			double(stat.m_oStat.m_lAlphabet)/1024.0/1024.0/8.0,(stat.m_oStat.m_lAlphabet>>3).string());
		m_IF.printf("%10.3f MiB (%12s Bytes) Huffman tables.\n",
			double(stat.m_oStat.m_lHuffmanTables)/1024.0/1024.0/8.0,(stat.m_oStat.m_lHuffmanTables>>3).string());
		m_IF.printf("%10.3f MiB (%12s Bytes) table switching.\n",
			double(stat.m_oStat.m_lTableSwitch)/1024.0/1024.0/8.0,(stat.m_oStat.m_lTableSwitch>>3).string());
		m_IF.printf("%10.3f MiB (%12s Bytes) payload data.\n",
			double(stat.m_oStat.m_lHuffmanData)/1024.0/1024.0/8.0,(stat.m_oStat.m_lHuffmanData>>3).string());
		m_IF.printf("%10.3f MiB (%12s Bytes) in total.\n",
			double(stat.m_oStat.m_lOverhead+stat.m_oStat.m_lAlphabet+stat.m_oStat.m_lHuffmanTables+stat.m_oStat.m_lTableSwitch+stat.m_oStat.m_lHuffmanData)/1024.0/1024.0/8.0,
			((stat.m_oStat.m_lOverhead+stat.m_oStat.m_lAlphabet+stat.m_oStat.m_lHuffmanTables+stat.m_oStat.m_lTableSwitch+stat.m_oStat.m_lHuffmanData)>>3).string());
		if (tf > 0) {
			m_IF.bprintf("\n  * Totals:\n");
			m_IF.printf("      %9.3f KiB per frame on average.\n",tb/1024.0/tf);
			m_IF.printf("      %9.3f bits per coodinate on average.\n",tb/tf/atc/3.0*8.0);
			m_IF.printf("      Compression ratio of %.3f : 1\n",(double)insize/tb);
		}
		if (tf2 > 0) {
			m_IF.bprintf("\n  * Starting from step %d:\n",morder+1);
...
_cagain:
			...
			if (err) {
				if (ofc2 != 0) {
					m_IF.eprintf("Errors occured. Compressing frame again with order zero.\n");
					err = false;
					ofc2 = 0;
					goto _again;
				} else {
					m_IF.eprintf("Errors occured despite of order zero. Aborting.\n");
					return false;
				}
			}
