...
CGraceDataset* CGraceGraph::Dataset(int i)
{
	...
	{
		eprintf("CGraceGraph::Dataset(): Error: %d/%d.\n",i,m_oaDatasets.GetSize());
		abort();
	}
	return (CGraceDataset*)m_oaDatasets[m_oaDatasets.GetSize()-1];
}
...
//	g = m_pGraph;
	...
	if ( m_bSymbol )
	{
		mfprintf(a,"@    s%d symbol 1\n",m_iNumber);
		mfprintf(a,"@    s%d symbol size 0.500000\n",m_iNumber);
		...
		mfprintf(a,"@    s%d symbol fill pattern 1\n",m_iNumber);
		...
		mfprintf(a,"@    s%d line linestyle 0\n",m_iNumber);
...
void CGrace::FindMinMaxVal()
{
	...
	g->m_fMinValX = (double)9E20;
	g->m_fMinValY = (double)9E20;
	g->m_fMaxValX = (double)-9E20;
	g->m_fMaxValY = (double)-9E20;
	...
	for (z=0;z<g->m_oaDatasets.GetSize();z++)
	{
		...
		for (z2=0;z2<gd->m_faValues.GetSize()/2;z2++)
		{
			if (gd->m_faValues[z2*2] > g->m_fMaxValX)
				g->m_fMaxValX = (double)gd->m_faValues[z2*2];
			if (gd->m_faValues[z2*2] < g->m_fMinValX)
				g->m_fMinValX = (double)gd->m_faValues[z2*2];
			if (gd->m_faValues[z2*2+1] > g->m_fMaxValY)
				g->m_fMaxValY = (double)gd->m_faValues[z2*2+1];
			if (gd->m_faValues[z2*2+1] < g->m_fMinValY)
				g->m_fMinValY = (double)gd->m_faValues[z2*2+1];
		}
...
void CGrace::SetRangeX(double mi, double ma)
{
	...
	g->m_fMinRangeX = (double)mi;
	g->m_fMaxRangeX = (double)ma;
	g->m_fTickMajorX = majorticks(g->m_fMinRangeX,g->m_fMaxRangeX);
	g->m_iTickPrecX = (int)MAX(0,1-int(ceil(log10(g->m_fTickMajorX))));
}
...
void CGrace::SetRangeY(double mi, double ma)
{
	...
	g->m_fMinRangeY = (double)mi;
	g->m_fMaxRangeY = (double)ma;
	g->m_fTickMajorY = majorticks(g->m_fMinRangeY,g->m_fMaxRangeY);
	g->m_iTickPrecY = (int)MAX(0,1-int(ceil(log10(g->m_fTickMajorY))));
}
...
{
	SetSetLineColor(set,(unsigned char)(col & 0x100), (unsigned char)((col >> 8) & 0x100), (unsigned char)(col >> 16));
}


void CGrace::SetSetLineWidth(int set, double width)
{
	CGraceDataset* gd;
	CGraceGraph *g;
...
void CGrace::SetViewport(double x1, double y1, double x2, double y2)
{
	CGraceGraph *g;
