...
//	char buf[32768], buf2[32768];
	...
	CElement *el;
	CxIntArray *pia, ia;
	CxDVector3 veca(0.0, 0.0, 0.0), vecb(0.0, 0.0, 0.0), vecc(0.0, 0.0, 0.0);
...
//				mprintf("%s merged to %s\n",at->m_sName,at->m_pMergedTo->m_sName);
			...
			if (tb)
				mprintf(", ");
			tb = true;
			mprintf("%s",(const char*)at->m_sName);
			ti++;
			ti2 = z;
		}
		mprintf("\n\n");
		...
		{  // Only Wannier centers unrecognized.
			mprintf("    Possibly, atom type \"X\" represents Wannier centers. If so, do not assign data to them.\n\n");
			tb2 = false; 
		} else tb2 = true;
		...
		if (AskYesNo("    Do you want to assign atom data to them (y/n)? [%s] ",tb2,tb2?"yes":"no"))
		{
			if (tb2)
			{
				if (tbnew)
					goto _manu;
				if (AskYesNo("    Automatically rename all atoms to corresponding elements (recommended) (y/n)? [yes] ",true))
				{
					tbnew = true;
					mprintf("\n");
					tb2 = true;
...
//								strcpy(buf2,at->m_sName);
						...
						mprintf("    - Renaming %s to %s...\n",(const char*)at->m_sName,(const char*)buf2);
						...
						abort();
_unkX1:;
						at->m_iCount--; // Remove one from xAddAtom
...
//						strcpy(buf2,at->m_sName);
				...
				{
_unk5:
					AskString("    Enter new name for \"%s\": [%s] ",&buf,(const char*)buf2,(const char*)at->m_sName,(const char*)buf2);
			//		if (strlen(buf) > 7)
			//		{
			//			eprintf("Atom labels may have maximum length of 7 characters.\n");
			//			goto _unk5;
			//		}
					if (ContainsDigitOrSpecial(buf))
					{
						eprintf("Digits or special characters in element labels not allowed.\n");
						goto _unk5;
					}
					...
					{
						xAddAtom(buf,false);
						if (AskYesNo("    Element %s is known. Merge %s atoms into %s (y/n)? [yes] ",true,(const char*)buf,(const char*)at->m_sName,(const char*)buf))
						{
							for (z2=0;z2<g_oaAtoms.GetSize();z2++)
							{
								if (((CAtom*)g_oaAtoms[z2])->m_pElement == el)
...
_unkdone:
			...
			for (z=0;z<g_oaAtoms.GetSize();z++)
			{
				...
				//if (((CAtom*)g_oaAtoms[z])->m_pMergedTo != NULL)
				//	mprintf(" (merged to %s)",(const char*)((CAtom*)g_oaAtoms[z])->m_pMergedTo->m_sName);
			}
			mprintf("\n");

		} else { // END IF CHANGE UNKNOWN
			...
			for (z=0;z<g_oaAtoms.GetSize();z++) {
				...
				if (AskYesNo("    Do you want to hide atom type \"%s\" (y/n)? [yes] ",true,(const char*)((CAtom*)g_oaAtoms[z])->m_sName))
					((CAtom*)g_oaAtoms[z])->m_bExclude = true;
			}
	...
	if (g_bUnknownElements) {
		...
		for (z=0;z<g_oaAtoms.GetSize();z++) {
			...
			if (((CAtom*)g_oaAtoms[z])->m_bExclude)
				continue;
			if (strlen(((CAtom*)g_oaAtoms[z])->m_sName) < 2)
				continue;

			buf.sprintf("%c",((CAtom*)g_oaAtoms[z])->m_sName[0]);

			for (z2=0;z2<g_oaElements.GetSize();z2++) {
				el = (CElement*)g_oaElements[z2];
				if (mystricmp((const char*)buf,el->m_sLabel) == 0)
					goto _ef;
			}
...
_ef:
			if (!tb) {
				tb = true;
				mprintf("    It frequently happens that force field molecular dynamics software uses atom labels\n");
				mprintf("    such as \"CR\" for carbon, which is then erroneously considered as chromium by TRAVIS.\n\n");
			}

			if (!AskYesNo("    Does your system really contain the chemical element \"%s\" (y), or are these \"%s\" atoms (n)? [no] ",false,(const char*)((CAtom*)g_oaAtoms[z])->m_sName,(const char*)buf)) {

				b = true;
				mprintf("\n      Renaming %d \"%s\" atoms to \"%s\"...\n\n",((CAtom*)g_oaAtoms[z])->m_iCount,(const char*)((CAtom*)g_oaAtoms[z])->m_sName,(const char*)buf);
	...
	for (z=0;z<g_oaAtoms.GetSize();z++)
	{
		...
		for (z2=z+1;z2<g_oaAtoms.GetSize();z2++)
		{
			if (((CAtom*)g_oaAtoms[z2])->m_pMergedTo != NULL)
				continue;
			if (((CAtom*)g_oaAtoms[z2])->m_bExclude)
				continue;
			...
			if (fabs(((CAtom*)g_oaAtoms[z])->m_pElement->m_fMass - ((CAtom*)g_oaAtoms[z2])->m_pElement->m_fMass) < 0.01)
			{
				if (!tb)
				{
					...
					mprintf(WHITE,"      Mass=%.2f: %s, %s",((CAtom*)g_oaAtoms[z])->m_pElement->m_fMass,(const char*)((CAtom*)g_oaAtoms[z])->m_sName,(const char*)((CAtom*)g_oaAtoms[z2])->m_sName);
					ia.Add(z);
					ia.Add(z2);
				} else
				{
					mprintf(WHITE,", %s",(const char*)((CAtom*)g_oaAtoms[z2])->m_sName);
				}
			}
		}
		if (tb)
...
_next:;
	...
	if (((g_iTrajFormat == 5) || (g_iTrajFormat == 7)) && (g_TimeStep.m_pVolumetricData != NULL)) {
		mprintf("    First frame from volumetric data trajectory contains these cell vectors:\n");
		mprintf("      A = ( %10.4f | %10.4f | %10.4f ) pm\n",g_fCubeXVector[0]*g_iCubeRes[0]*LEN_AU2PM,g_fCubeXVector[1]*g_iCubeRes[0]*LEN_AU2PM,g_fCubeXVector[2]*g_iCubeRes[0]*LEN_AU2PM);
		mprintf("      B = ( %10.4f | %10.4f | %10.4f ) pm\n",g_fCubeYVector[0]*g_iCubeRes[1]*LEN_AU2PM,g_fCubeYVector[1]*g_iCubeRes[1]*LEN_AU2PM,g_fCubeYVector[2]*g_iCubeRes[1]*LEN_AU2PM);
		mprintf("      C = ( %10.4f | %10.4f | %10.4f ) pm\n",g_fCubeZVector[0]*g_iCubeRes[2]*LEN_AU2PM,g_fCubeZVector[1]*g_iCubeRes[2]*LEN_AU2PM,g_fCubeZVector[2]*g_iCubeRes[2]*LEN_AU2PM);
		mprintf("\n");

		tb = false;
		if (g_fCubeXVector[0] < 0) {
			if (!tb) {
				tb = true;
				mprintf("    Some diagonal elements of the cell matrix have negative values.\n");
			...
			g_bFlipCellVectorX = AskYesNo("    Mirror at Y-Z plane such that AX becomes positive (y/n)? [yes] ",true);
		}
		if (g_fCubeYVector[1] < 0) {
			if (!tb) {
				tb = true;
				mprintf("    Some diagonal elements of the cell matrix have negative values.\n");
				mprintf("    This is quite uncommon and can cause problems, e.g. in Voronoi integration.\n");
			...
			g_bFlipCellVectorY = AskYesNo("    Mirror at X-Z plane such that BY becomes positive (y/n)? [yes] ",true);
		}
		if (g_fCubeZVector[2] < 0) {
			if (!tb) {
				tb = true;
				mprintf("    Some diagonal elements of the cell matrix have negative values.\n");
			...
			g_bFlipCellVectorZ = AskYesNo("    Mirror at X-Y plane such that CZ becomes positive (y/n)? [yes] ",true);
		}
		if (tb) {
			mprintf("\n");
			mprintf("    Flipping vectors...\n\n");
			if (g_bFlipCellVectorX) {
				if (g_fCubeXVector[0] != 0)
					g_fCubeXVector[0] *= -1.0;
				if (g_fCubeYVector[0] != 0)
					g_fCubeYVector[0] *= -1.0;
				if (g_fCubeZVector[0] != 0)
					g_fCubeZVector[0] *= -1.0;
				if (g_mCubeCell(0,0) != 0)
					g_mCubeCell(0,0) *= -1.0;
				if (g_mCubeCell(1,0) != 0)
					g_mCubeCell(1,0) *= -1.0;
				if (g_mCubeCell(2,0) != 0)
					g_mCubeCell(2,0) *= -1.0;
			}
			if (g_bFlipCellVectorY) {
				if (g_fCubeXVector[1] != 0)
					g_fCubeXVector[1] *= -1.0;
				if (g_fCubeYVector[1] != 0)
					g_fCubeYVector[1] *= -1.0;
				if (g_fCubeZVector[1] != 0)
					g_fCubeZVector[1] *= -1.0;
				if (g_mCubeCell(0,1) != 0)
					g_mCubeCell(0,1) *= -1.0;
				if (g_mCubeCell(1,1) != 0)
					g_mCubeCell(1,1) *= -1.0;
				if (g_mCubeCell(2,1) != 0)
					g_mCubeCell(2,1) *= -1.0;
			}
			if (g_bFlipCellVectorZ) {
				if (g_fCubeXVector[2] != 0)
					g_fCubeXVector[2] *= -1.0;
				if (g_fCubeYVector[2] != 0)
					g_fCubeYVector[2] *= -1.0;
				if (g_fCubeZVector[2] != 0)
					g_fCubeZVector[2] *= -1.0;
				if (g_mCubeCell(0,2) != 0)
					g_mCubeCell(0,2) *= -1.0;
				if (g_mCubeCell(1,2) != 0)
					g_mCubeCell(1,2) *= -1.0;
				if (g_mCubeCell(2,2) != 0)
					g_mCubeCell(2,2) *= -1.0;
			}
			mprintf("    First frame from volumetric data trajectory contains these cell vectors:\n");
			mprintf("      A = ( %10.4f | %10.4f | %10.4f ) pm\n",g_fCubeXVector[0]*g_iCubeRes[0]*LEN_AU2PM,g_fCubeXVector[1]*g_iCubeRes[0]*LEN_AU2PM,g_fCubeXVector[2]*g_iCubeRes[0]*LEN_AU2PM);
...
/*			mprintf("      A = ( %.10e | %.10e | %.10e ) pm\n",g_fCubeXVector[0]*g_iCubeRes[0]*LEN_AU2PM,g_fCubeXVector[1]*g_iCubeRes[0]*LEN_AU2PM,g_fCubeXVector[2]*g_iCubeRes[0]*LEN_AU2PM);
			mprintf("      B = ( %.10e | %.10e | %.10e ) pm\n",g_fCubeYVector[0]*g_iCubeRes[1]*LEN_AU2PM,g_fCubeYVector[1]*g_iCubeRes[1]*LEN_AU2PM,g_fCubeYVector[2]*g_iCubeRes[1]*LEN_AU2PM);
			mprintf("      C = ( %.10e | %.10e | %.10e ) pm\n",g_fCubeZVector[0]*g_iCubeRes[2]*LEN_AU2PM,g_fCubeZVector[1]*g_iCubeRes[2]*LEN_AU2PM,g_fCubeZVector[2]*g_iCubeRes[2]*LEN_AU2PM);
			mprintf("\n");
*/		}
	}
...
/*	mprintf("    The advanced mode includes many additional options which are quite powerful, yet possibly\n");
	...
	if ((g_iTrajFormat == 5) || (g_iTrajFormat == 7)) {
		if (!g_bAdvanced1 && ((g_fCubeXVector[1] != 0) || (g_fCubeXVector[2] != 0) || (g_fCubeYVector[0] != 0) || (g_fCubeYVector[2] != 0) || (g_fCubeZVector[0] != 0) || (g_fCubeZVector[1] != 0))) {
			mprintf(WHITE,"    Warning:");
			mprintf(" Cell seems to be non-orthorhombic. Activating advanced mode.\n\n");
			g_bAdvanced1 = true;
		}
	}
	...
	{
		mprintf("    TRAVIS can read orthorhombic cell edge lengths from the comment line of the XYZ file.\n");
		mprintf("    The format needs to be \"x y z\" (space separated). Lengths in Angstrom.\n\n");
		mprintf("    The current comment line is \"%s\".\n\n",g_TimeStep.m_pComment);
		if (AskYesNo("    Are the three numbers in the trajectory's comment line cell geometry data in this format (y/n)? [yes] ",true))
		{
			mprintf("\n    Extracting cell geometry...\n");
			ExtractXYZCellGeometry3(g_TimeStep.m_pComment);
			mprintf("\n");
		} else
...
//		}
	...
	if (g_bPeriodic)
	{
		...
		if (g_bAdvanced1)
		{
			if ((g_fCubeXVector[1] != 0) || (g_fCubeXVector[2] != 0) || (g_fCubeYVector[0] != 0) || (g_fCubeYVector[2] != 0) || (g_fCubeZVector[0] != 0) || (g_fCubeZVector[1] != 0))
				g_bBoxNonOrtho = AskYesNo("    Is the simulation cell non-orthorhombic (cell angles other than 90 deg) (y/n)? [yes] ",true);
			else
...
_celldone:
			...
			if (!g_mBoxToOrtho.Invert()) {
				eprintf("Error: Encountered singular cell matrix (cell volume is zero).\n");
				abort();
			}
			...
			if (g_bPeriodicX && g_bPeriodicY && g_bPeriodicZ)
			{
				...
				mprintf("    The overall box density is %.6f g/cm^3.\n",tf*tf*tf/g_fBoxX/g_fBoxY/g_fBoxZ);
				if (tf*tf*tf/g_fBoxX/g_fBoxY/g_fBoxZ > 15.0)
				{
					mprintf("\n");
		...
		{
			if (AskYesNo("    Enter three cell vectors (y) or three edge lengths and angles (n)? [%s] ",((g_iTrajFormat == 5) || (g_iTrajFormat == 7)),((g_iTrajFormat == 5) || (g_iTrajFormat == 7))?"yes":"no"))
			{
				mprintf("\n");

_cellagain:
				g_mBoxFromOrtho(0,0) = AskFloat_ND("    Enter X component of cell vector A in pm: ");
				if (g_bFlipCellVectorX && (g_mBoxFromOrtho(0,0) < 0)) {
					eprintf("You decided to mirror at the Y-Z plane. Please enter the X components with inverse signs.\n\n");
					goto _cellagain;
				}
				...
				if (g_bFlipCellVectorY && (g_mBoxFromOrtho(1,1) < 0)) {
					eprintf("You decided to mirror at the X-Z plane. Please enter the Y components with inverse signs.\n\n");
					goto _cellagain;
				}
				...
				if (g_bFlipCellVectorZ && (g_mBoxFromOrtho(2,2) < 0)) {
					eprintf("You decided to mirror at the X-Y plane. Please enter the Z components with inverse signs.\n\n");
					goto _cellagain;
				}
		...
		if ((g_iTrajFormat == 5) || ((g_iTrajFormat == 7) && (g_TimeStep.m_pVolumetricData != NULL))) {
			...
			if (g_bBoxNonOrtho) {
				double ang = Angle_Deg(CxDVector3(g_mCubeCell(0, 0), g_mCubeCell(0, 1), g_mCubeCell(0, 2)), veca);
				if (ang > 1.0) {
					eprintf("The direction of cell vector A does not match the grid data in the cube file (the angle is %.2f deg).\nPlease check the cell vectors.\n", ang);
					abort();
				}
				ang = Angle_Deg(CxDVector3(g_mCubeCell(1, 0), g_mCubeCell(1, 1), g_mCubeCell(1, 2)), vecb);
				if (ang > 1.0) {
					eprintf("The direction of cell vector B does not match the grid data in the cube file (the angle is %.2f deg).\nPlease check the cell vectors.\n", ang);
					abort();
				}
				ang = Angle_Deg(CxDVector3(g_mCubeCell(2, 0), g_mCubeCell(2, 1), g_mCubeCell(2, 2)), vecc);
				if (ang > 1.0) {
					eprintf("The direction of cell vector C does not match the grid data in the cube file (the angle is %.2f deg).\nPlease check the cell vectors.\n", ang);
					abort();
				}
			}
			mprintf("\n");
			double diff;
			if (g_bBoxNonOrtho) {
				CxDVector3 diffvec = CxDVector3(g_fCubeXVector[0], g_fCubeXVector[1], g_fCubeXVector[2]) * (double)LEN_AU2PM * g_TimeStep.m_pVolumetricData->m_iRes[0];
				diff = diffvec.GetLength() - veca.GetLength();
			} else {
				diff = g_fBoxX - g_fCubeXStep * LEN_AU2PM * g_TimeStep.m_pVolumetricData->m_iRes[0];
			}
			if (fabs(diff) > 0.1) {
				mprintf("    The X cell vector does not match the grid data in the cube file (the difference is %.3f pm).\n", diff);
				...
				for (i = 1; i < g_iCubeXStride; i++) {
					if (g_bBoxNonOrtho) {
						CxDVector3 diffvec = CxDVector3(g_fCubeXVector[0], g_fCubeXVector[1], g_fCubeXVector[2]) * (double)LEN_AU2PM * (g_TimeStep.m_pVolumetricData->m_iRes[0] * g_iCubeXStride - i) / g_iCubeXStride;
						diff = CxDVector3(g_fCubeXVector[0], g_fCubeXVector[1], g_fCubeXVector[2]).GetLength() - veca.GetLength();
					} else {
...
// 				}
				...
				mprintf("    X cell vector matches grid data in cube file (difference is %.3f pm).\n", diff);
			}
			
			if (g_bBoxNonOrtho) {
				CxDVector3 diffvec = CxDVector3(g_fCubeYVector[0], g_fCubeYVector[1], g_fCubeYVector[2]) * (double)LEN_AU2PM * g_TimeStep.m_pVolumetricData->m_iRes[1];
			...
			if (fabs(diff) > 0.1) {
				...
				for (i = 1; i < g_iCubeYStride; i++) {
					if (g_bBoxNonOrtho) {
						CxDVector3 diffvec = CxDVector3(g_fCubeYVector[0], g_fCubeYVector[1], g_fCubeYVector[2]) * (double)LEN_AU2PM * (g_TimeStep.m_pVolumetricData->m_iRes[1] * g_iCubeYStride - i) / g_iCubeXStride;
						diff = CxDVector3(g_fCubeYVector[0], g_fCubeYVector[1], g_fCubeYVector[2]).GetLength() - vecb.GetLength();
					} else {
...
// 				}
				...
				mprintf("    Y cell vector matches grid data in cube file (difference is %.3f pm).\n", diff);
			}
			
			if (g_bBoxNonOrtho) {
				CxDVector3 diffvec = CxDVector3(g_fCubeZVector[0], g_fCubeZVector[1], g_fCubeZVector[2]) * (double)LEN_AU2PM * g_TimeStep.m_pVolumetricData->m_iRes[2];
			...
			if (fabs(diff) > 0.1) {
				...
				for (i = 1; i < g_iCubeZStride; i++) {
					if (g_bBoxNonOrtho) {
						CxDVector3 diffvec = CxDVector3(g_fCubeZVector[0], g_fCubeZVector[1], g_fCubeZVector[2]) * (double)LEN_AU2PM * (g_TimeStep.m_pVolumetricData->m_iRes[2] * g_iCubeZStride - i) / g_iCubeZStride;
						diff = CxDVector3(g_fCubeZVector[0], g_fCubeZVector[1], g_fCubeZVector[2]).GetLength() - vecc.GetLength();
					} else {
...
// 				}
				...
				mprintf("    Z cell vector matches grid data in cube file (difference is %.3f pm).\n", diff);
			}
			...
			if (g_bBoxNonOrtho) {
				...
				g_TimeStep.m_pVolumetricData->m_fMaxVal[0] = sqrt(g_mCubeCell(0, 0) * g_mCubeCell(0, 0) + g_mCubeCell(0, 1) * g_mCubeCell(0, 1) + g_mCubeCell(0, 2) * g_mCubeCell(0, 2));
				g_TimeStep.m_pVolumetricData->m_fMaxVal[1] = sqrt(g_mCubeCell(1, 0) * g_mCubeCell(1, 0) + g_mCubeCell(1, 1) * g_mCubeCell(1, 1) + g_mCubeCell(1, 2) * g_mCubeCell(1, 2));
				g_TimeStep.m_pVolumetricData->m_fMaxVal[2] = sqrt(g_mCubeCell(2, 0) * g_mCubeCell(2, 0) + g_mCubeCell(2, 1) * g_mCubeCell(2, 1) + g_mCubeCell(2, 2) * g_mCubeCell(2, 2));
			} else {
				if (fabs(g_fCubeXStep - g_fBoxX / (g_TimeStep.m_pVolumetricData->m_iRes[0] * g_iCubeXStride - g_iCubeXMismatch) / LEN_AU2PM) > 0.1 || fabs(g_fCubeYStep - g_fBoxY / (g_TimeStep.m_pVolumetricData->m_iRes[1] * g_iCubeYStride - g_iCubeYMismatch) / LEN_AU2PM) > 0.1 || fabs(g_fCubeZStep - g_fBoxZ / (g_TimeStep.m_pVolumetricData->m_iRes[2] * g_iCubeZStride - g_iCubeZMismatch) / LEN_AU2PM) > 0.1) {
...
_celldefined:
	...
	if (firstmetal) {
		...
		for (z=0;z<g_oaAtoms.GetSize()-1;z++) {
			if (((CAtom*)g_oaAtoms[z])->m_pMergedTo != NULL)
				continue;
			if (IsElementMetal((const char*)((CAtom*)g_oaAtoms[z])->m_pElement->m_sLabel))
				tb = true;
		}
		if (tb) {
			mprintf("\n");
			...
			for (z=0;z<g_oaAtoms.GetSize()-1;z++) {
				if (((CAtom*)g_oaAtoms[z])->m_pMergedTo != NULL)
					continue;
				for (z2=0;z2<z;z2++)
					if (((CAtom*)g_oaAtoms[z])->m_pElement == ((CAtom*)g_oaAtoms[z2])->m_pElement)
						goto _nextmetal;
				if (IsElementMetal((const char*)((CAtom*)g_oaAtoms[z])->m_pElement->m_sLabel)) {
					if (AskYesNo("      Exclude %s atoms from bond recognition (y/n)? [yes] ",true,(const char*)((CAtom*)g_oaAtoms[z])->m_pElement->m_sLabel))
						((CAtom*)g_oaAtoms[z])->m_pElement->m_fRadius = 0;
				}
	_nextmetal:;
			}
		}
		...
		for (z=0;z<g_oaAtoms.GetSize()-1;z++) {
			if (((CAtom*)g_oaAtoms[z])->m_pMergedTo != NULL)
				continue;
			if (IsElementNobleGas((const char*)((CAtom*)g_oaAtoms[z])->m_pElement->m_sLabel))
				tb = true;
		}
		if (tb) {
			mprintf("\n");
...
_molbegin:
	...
	if (g_bReactive) {
		...
		if (g_pReactEngine == NULL)
			g_pReactEngine = new CReactiveEngine();

		g_pReactEngine->Parse();
	...
	//	a = fopen(g_sInputTraj,"rb");
		...
		/*		if (!SeekInputTrajectory(g_iScanMolStep))
					return false;*/
...
_reactivedone:
	...
	{
		mprintf("\n");
		eprintf("You don't have any atom/molecule in your simulation. This is probably not what you want.\n");
	...
	#ifdef USE_OPENBABEL
		mprintf("\n");
		if (AskYesNo("    Obtain canonical SMILES strings from OpenBabel for the molecules (y/n)? [yes] ",true)) {
			mprintf("\n");
			for (z=0;z<g_oaMolecules.GetSize();z++) {
				...
				buf.sprintf("mol%d.pdb",z+1);
				mprintf("      Writing PDB file to \"%s\"...\n",(const char*)buf);

				g_TimeStep.ExportSingleMolecule_PDB(sm,(const char*)buf);

				buf.sprintf("obabel -ipdb mol%d.pdb -ocan -Omol%d_smiles.txt",z+1,z+1);
				mprintf("      Executing OpenBabel (\"%s\")...\n",(const char*)buf);

				(void)system((const char*)buf);

				buf.sprintf("mol%d_smiles.txt",z+1);
				mprintf("      Reading output file...\n");

				if (!FileExist((const char*)buf)) {
					eprintf("      Output file not present. OpenBabel run did not work.\n");
					continue;
				}

				a = fopen((const char*)buf,"rt");
				(void)buf.fgets(1024,a);
				fclose(a);

				if (buf.FindFirst(' ') != -1)
					buf.GetWritePointer()[buf.FindFirst(' ')] = 0;
				if (buf.FindFirst('\t') != -1)
					buf.GetWritePointer()[buf.FindFirst('\t')] = 0;

				m->m_sSMILES = buf;

				mprintf("      Read %d characters from SMILES string.\n",m->m_sSMILES.GetLength());
...
_matrixagain:
	...
	{
		m = (CMolecule*)g_oaMolecules[z];
		mprintf(WHITE,"  - Molecule %d: %s ",z+1,m->m_sName);
		mprintf("(%d piece%s, %.2f g/mol)\n",m->m_laSingleMolIndex.GetSize(),(m->m_laSingleMolIndex.GetSize()>1)?"s":"",m->m_fMass);
		...
		if (m->m_sSMILES.GetLength() != 0)
			mprintf("      Canonical SMILES: %s\n",(const char*)m->m_sSMILES);
		if (((CMolecule*)g_oaMolecules[z])->m_oaRingAtoms.GetSize() != 0)
		{
			if (((CMolecule*)g_oaMolecules[z])->m_oaRingAtoms.GetSize() == 1)
				mprintf(WHITE,"      Detected %d ring:\n",m->m_oaRingAtoms.GetSize());
			else
				mprintf(WHITE,"      Detected %d rings:\n",m->m_oaRingAtoms.GetSize());

			for (z2=0;z2<m->m_oaRingAtoms.GetSize();z2++)
			{
				mprintf(WHITE,"        %2d.) %2d-ring: ",z2+1,((CxIntArray*)m->m_oaRingAtoms[z2])->GetSize());
				if (((CxIntArray*)m->m_oaRingAtoms[z2])->GetSize() > 16)
				{
					for (z3=0;z3<5;z3++)
					{
...
_modagain:
		...
		switch(AskRangeInteger("    Please select: [done] ",1,3,0))
		{
			case 1: // Change radii
				...
				for (z=0;z<g_oaAtoms.GetSize()-1;z++)
					mprintf("      - %-2s    %5.1f pm\n",(const char*)((CAtom*)g_oaAtoms[z])->m_sName,((CAtom*)g_oaAtoms[z])->m_pElement->m_fRadius*g_fBondFactor);
				mprintf("\n");
_arnew:
				...
				for (z=0;z<g_oaAtoms.GetSize()-1;z++)
				{
					if (mystricmp(buf,((CAtom*)g_oaAtoms[z])->m_sName) == 0)
					{
						((CAtom*)g_oaAtoms[z])->m_pElement->m_fRadius = AskFloat("    Please enter new bond radius for %s in pm: [%.1f] ",((CAtom*)g_oaAtoms[z])->m_pElement->m_fRadius*g_fBondFactor,(const char*)((CAtom*)g_oaAtoms[z])->m_sName,((CAtom*)g_oaAtoms[z])->m_pElement->m_fRadius*g_fBondFactor) / g_fBondFactor;
						if (((CAtom*)g_oaAtoms[z])->m_pElement->m_fRadius == 0) {
							mprintf("\n      Atom %s will form no bonds now.\n\n",(const char*)((CAtom*)g_oaAtoms[z])->m_sName);
							((CAtom*)g_oaAtoms[z])->m_bExclude = AskYesNo("    Also remove this atom type from the system (y/n)? [no] ",false);
						}
						mprintf("\n");
						tb = true;
						goto _arnew;
...
_renfound:
		...
		//			goto _renfound;
		//		}
				if (ContainsDigitOrSpecial(buf2))
				{
...
//_ren1:;
				...
				} else
				{
					...
					at->m_pElement->m_fRadius = AskFloat("    Please enter covalent radius in pm: [100.0] ",100.0);
			...
			//		sprintf(at->m_sName,"%s",(const char*)buf2);
					...
					at->m_pElement->m_sLabel = new char[buf2.GetLength()+1];
					strcpy(at->m_pElement->m_sLabel,(const char*)buf2);
					g_oaElements.Add(at->m_pElement);
				}

	/*			for (z2=0;z2<g_oaAtoms.GetSize();z2++) {
					if (mystricmp(buf2,((CAtom*)g_oaAtoms[z])->m_sName) == 0) {
						mprintf("    Element %s is known. Merging all %s atoms into %s ...\n",(const char*)buf2,(const char*)buf,(const char*)buf2);
						at = (CAtom*)g_oaAtoms[z2];
						goto _ren1;
					}
				}
...
//				strcat(buf,")? ");
				...
				{
					mprintf("* Atom type %s\n",(const char*)((CAtom*)g_oaAtoms[mol->m_baAtomIndex[z]])->m_sName);
					...
					{
		_renumberagain:
						ti2 = AskRangeInteger("    Which number should %s%d bear? [%d] ",1,mol->m_waAtomCount[z],z2+1,(const char*)((CAtom*)g_oaAtoms[mol->m_baAtomIndex[z]])->m_sName,z2+1,z2+1);
						for (z3=0;z3<z2;z3++)
							if (((CxIntArray*)mol->m_oaNewNumbers[z])->GetAt(z3) == ti2-1)
							{
								eprintf("This number already was chosen for %s%d!\n",(const char*)((CAtom*)g_oaAtoms[mol->m_baAtomIndex[z]])->m_sName,z3+1);
								goto _renumberagain;
							}
	...
	for (z0=0;z0<g_oaMolecules.GetSize();z0++)
	{
		...
		for (z=0;z<va->m_oCenterAtoms.m_iAtomGes;z++)
			va->m_faWeight[z] = 1.0;
		va->m_fGesWeight = (double)va->m_oCenterAtoms.m_iAtomGes;
	}
		...
		if (m->m_oaRingAtoms.GetSize() != 0)
		{
			...
			} else
			{
				...
				for (z=0;z<m->m_oaRingAtoms.GetSize();z++)
				{
					...
					{
						mprintf("%s%d",(const char*)((CAtom*)g_oaAtoms[m->m_baAtomIndex[((CxIntArray*)m->m_oaRingAtomTypes[z])->GetAt(z3)]])->m_sName,((CxIntArray*)m->m_oaRingAtoms[z])->GetAt(z3)+1);
...
//			strcat(buf,")? ");
			...
			va->m_iMode = (unsigned char)AskRangeInteger("    Define v.a. as center (0) or through distance/angle/dihedral (1)? [center] ",0,1,0);
			if (va->m_iMode == 0)
			{
	_vaatoms:
...
_vawagain:
					...
					for (z=0;z<va->m_oCenterAtoms.m_baAtomType.GetSize();z++)
					{
						for (z2=0;z2<((CxIntArray*)va->m_oCenterAtoms.m_oaAtoms[z])->GetSize();z2++)
						{
							tf = AskFloat("      Enter the weight for %s%d: [1] ",1.0,((CAtom*)g_oaAtoms[va->m_oCenterAtoms.m_baRealAtomType[z]])->m_pElement->m_sLabel, ((CxIntArray *)va->m_oCenterAtoms.m_oaAtoms[z])->GetAt(z2)+1);
							va->m_faWeight[z3] = tf;
							tf2 += tf;
							z3++;
						}
	...
	_vaangle:
				mprintf("    Enter 3rd atom for angle #%d-%s%d- (e.g. C2)? ",va->m_iMolVirtAtom+1,(const char*)((CAtom*)g_oaAtoms[va->m_iRealAtomType[0]])->m_sName,va->m_iAtom[0]+1);
				inpprintf("! Enter 3rd atom for angle #%d-%s%d- (e.g. C2)?\n",va->m_iMolVirtAtom+1,(const char*)((CAtom*)g_oaAtoms[va->m_iRealAtomType[0]])->m_sName,va->m_iAtom[0]+1);
				myget(&buf);
	...
	_vadihedral:
				mprintf("    Enter 4th atom for dihedral #%d-%s%d-%s%d- (e.g. C2)? ",va->m_iMolVirtAtom+1,(const char*)((CAtom*)g_oaAtoms[va->m_iRealAtomType[0]])->m_sName,va->m_iAtom[0]+1,(const char*)((CAtom*)g_oaAtoms[va->m_iRealAtomType[1]])->m_sName,va->m_iAtom[1]+1);
				inpprintf("! Enter 4th atom for dihedral #%d-%s%d-%s%d- (e.g. C2)?\n",va->m_iMolVirtAtom+1,(const char*)((CAtom*)g_oaAtoms[va->m_iRealAtomType[0]])->m_sName,va->m_iAtom[0]+1,(const char*)((CAtom*)g_oaAtoms[va->m_iRealAtomType[1]])->m_sName,va->m_iAtom[1]+1);
				myget(&buf);
				...
				va->m_fValues[0] = AskFloat_ND("    Enter distance #%d-%s%d in pm: ",va->m_iMolVirtAtom+1,(const char*)((CAtom*)g_oaAtoms[va->m_iRealAtomType[0]])->m_sName,va->m_iAtom[0]+1);
				va->m_fValues[1] = AskFloat_ND("    Enter angle #%d-%s%d-%s%d in degree: ",va->m_iMolVirtAtom+1,(const char*)((CAtom*)g_oaAtoms[va->m_iRealAtomType[0]])->m_sName,va->m_iAtom[0]+1,(const char*)((CAtom*)g_oaAtoms[va->m_iRealAtomType[1]])->m_sName,va->m_iAtom[1]+1) * (double)Pi / 180.0;
				va->m_fValues[2] = AskFloat_ND("    Enter dihedral #%d-%s%d-%s%d-%s%d in degree: ",va->m_iMolVirtAtom+1,(const char*)((CAtom*)g_oaAtoms[va->m_iRealAtomType[0]])->m_sName,va->m_iAtom[0]+1,(const char*)((CAtom*)g_oaAtoms[va->m_iRealAtomType[1]])->m_sName,va->m_iAtom[1]+1,(const char*)((CAtom*)g_oaAtoms[va->m_iRealAtomType[2]])->m_sName,va->m_iAtom[2]+1) * (double)Pi / 180.0;
			}
				...
				for (z=0;z<g_oaMolecules.GetSize();z++)
				{
					...
					for (z2=0;z2<ag->m_baAtomType.GetSize();z2++)
					{
						...
						for (z4=0;z4<m2->m_baAtomIndex.GetSize();z4++)
						{
							...
							{
								m2->m_waAtomCount[z4] += (unsigned short)(((CxIntArray*)ag->m_oaAtoms[z2])->GetSize() * ia.GetSize());
						...
						m2->m_waAtomCount.Add((unsigned short)(((CxIntArray*)ag->m_oaAtoms[z2])->GetSize() * ia.GetSize()));
							...
							for (z5=0;z5<ia.GetSize();z5++)
							{
								...
								for (z6=0;z6<sm2->m_oaAtomOffset.GetSize();z6++)
								{
									for (z7=0;z7<((CxIntArray*)sm2->m_oaAtomOffset[z6])->GetSize();z7++)
									{
										...
										{
											eprintf("Internal error: Atom %s%d from %s[%d] is already in the pseudo-molecule; skipping.\n",(const char*)((CAtom*)g_oaAtoms[ag->m_baRealAtomType[z2]])->m_sName,((CxIntArray*)ag->m_oaAtoms[z2])->GetAt(z4)+1,m->m_sName,ia[z5]+1);
...
*/
			for (z=0;z<g_oaMolecules.GetSize();z++)
			{
				...
				if (AskYesNo("    Use atoms from molecule %d (%s) (y/n)? [no] ",false,z+1,m->m_sName))
				{
					...
					if (AskYesNo("      Use all atoms (y) from %s or only certain atoms (n)? [yes] ",true,m->m_sName))
					{
						ag->AddAllAtoms(m,false);
					} else
					{
	...
	_pseudomolil:
					...
					if (!tb) // Custom Labels
					{
						...
						{
	_pmlagain:
							AskString("      How should %s from %s be labeled? [%s] ",&buf,(const char*)((CAtom*)g_oaAtoms[ag->m_baRealAtomType[z2]])->m_sName,(const char*)((CAtom*)g_oaAtoms[ag->m_baRealAtomType[z2]])->m_sName,m->m_sName,(const char*)((CAtom*)g_oaAtoms[ag->m_baRealAtomType[z2]])->m_sName);
							if (ContainsDigitOrSpecial(buf))
							{
								eprintf("Digits and special characters in element labels not allowed.\n");
								goto _pmlagain;
							}
	...
	_pseudomold1:
							...
							for (z3=0;z3<m2->m_baAtomIndex.GetSize();z3++)
							{
								...
								{
									m2->m_waAtomCount[z3] += (unsigned short)(((CxIntArray*)ag->m_oaAtoms[z2])->GetSize() * ia.GetSize());
									for (z4=0;z4<((CxIntArray*)ag->m_oaAtoms[z2])->GetSize();z4++)
									{
										for (z5=0;z5<ia.GetSize();z5++)
										{
	...
	_pseudomolsk1:;
							...
							m2->m_baAtomIndex.Add((unsigned char)ti2);
							sm2->m_baAtomIndex.Add((unsigned char)ti2);
							m2->m_waAtomCount.Add((unsigned short)(((CxIntArray*)ag->m_oaAtoms[z2])->GetSize() * ia.GetSize()));
	...
	for (z0=0;z0<g_oaMolecules.GetSize();z0++)
	{
		for (z=0;z<((CMolecule*)g_oaMolecules[z0])->m_laVirtualAtoms.GetSize();z++)
		{
			...
			if (va->m_iMode < 2)
			{
				...
				if (va->m_iMode == 0)
				{
					if (!((CMolecule*)g_oaMolecules[z0])->m_bPseudo || (z > 1))
					{
						...
						for (z2=0;z2<va->m_oCenterAtoms.m_baAtomType.GetSize();z2++)
						{
							...
							{
								mprintf("      - %2s%-3d  Weight %.2f%c\n",(const char*)((CAtom*)g_oaAtoms[va->m_oCenterAtoms.m_baRealAtomType[z2]])->m_sName,((CxIntArray*)va->m_oCenterAtoms.m_oaAtoms[z2])->GetAt(z3)+1,va->m_faWeight[z4]/va->m_fGesWeight*100.0,'%');
								z4++;
							}
						}
				...
				{
					mprintf("      - Distance #%d-%s%d = %.2f pm\n",z+1,(const char*)((CAtom*)g_oaAtoms[va->m_iRealAtomType[0]])->m_sName,va->m_iAtom[0]+1,va->m_fValues[0]);
					mprintf("      - Angle #%d-%s%d-%s%d = %.2f degree\n",z+1,(const char*)((CAtom*)g_oaAtoms[va->m_iRealAtomType[0]])->m_sName,va->m_iAtom[0]+1,(const char*)((CAtom*)g_oaAtoms[va->m_iRealAtomType[1]])->m_sName,va->m_iAtom[1]+1,va->m_fValues[1]*180.0/Pi);
					mprintf("      - Dihedral #%d-%s%d-%s%d-%s%d = %.2f degree\n",z+1,(const char*)((CAtom*)g_oaAtoms[va->m_iRealAtomType[0]])->m_sName,va->m_iAtom[0]+1,(const char*)((CAtom*)g_oaAtoms[va->m_iRealAtomType[1]])->m_sName,va->m_iAtom[1]+1,(const char*)((CAtom*)g_oaAtoms[va->m_iRealAtomType[2]])->m_sName,va->m_iAtom[2]+1,va->m_fValues[2]*180.0/Pi);
				}
...
_fncinput:
	...
	if (g_bVDF || g_bRaman || g_bCombined || g_bBondACF || g_bVHDF || g_bACF || g_bUseVelocities || g_bUseForces || g_bMSD || g_bAggregation || g_bDLDisp || g_bDLDF || g_bDACF || g_bRDyn || g_bIRSpec || g_bTimeDiff || g_bDeriv || g_bNormalCoordinate || g_bPower || g_bVACFNew || g_bIR || g_bSetUpPolarizabilityCalc || g_bFDF || g_bVCD || g_bMagneticDipoleRestart)
	{
		g_fTimestepLength = AskFloat("    Enter the length of one trajectory time step in fs: [0.5] ",0.5);
		mprintf("\n");
	}
...
/*	if (g_bSFac)
	...
	if (g_bROA) {
		...
		try { g_pROAEngine = new CROAEngine(); } catch(...) { g_pROAEngine = NULL; }
		if (g_pROAEngine == NULL) NewException((double)sizeof(CROAEngine),__FILE__,__LINE__,__PRETTY_FUNCTION__);
	...
	if (g_bFixedPlProj) {
		...
		try { g_pFixedPlProj = new CFixedPlProj(); } catch(...) { g_pFixedPlProj = NULL; }
		if (g_pFixedPlProj == NULL) NewException((double)sizeof(CFixedPlProj),__FILE__,__LINE__,__PRETTY_FUNCTION__);
		
		if (!g_pFixedPlProj->Parse())
			return false;
	}

	if (g_bOrder) {

		g_pOrderEngine = new COrderEngine();

		if (!g_pOrderEngine->Parse(&g_TimeStep))
			return false;
	}
	...
	if (g_bAggregation)
	{
		...
		g_bDLDF = AskYesNo("    Compute Dimer Lifetime Distribution Functions (DLDFs) (y/n)? [no] ",false);
...
//			strcat(buf,")? ");
			buf.strcat(")? ");
			g_iFixMol = (char)AskRangeInteger_ND("%s",1,g_oaMolecules.GetSize(),(const char*)buf) - 1;
		} else g_iFixMol = 0;
	...
	if (g_bSaveRefEnv)
	{
		...
		if (((CMolecule*)g_oaMolecules[g_iFixMol])->m_laSingleMolIndex.GetSize() > 1)
			g_iSaveRefMol = (unsigned char)AskRangeInteger("    Which representative of the reference molecules to use (1-%d)? [1] ",1,((CMolecule*)g_oaMolecules[g_iFixMol])->m_laSingleMolIndex.GetSize(),1,((CMolecule*)g_oaMolecules[g_iFixMol])->m_laSingleMolIndex.GetSize()) - 1;
...
_clustagain:
		...
		for (z=0;z<g_iClusterCount-1;z++)
		{
			...
			if (ti3 == -1) {
				eprintf("Internal error in sorting cluster distribution.\n");
				abort();
			}
...
_ref3again:
		...
		for (z=1;z<3;z++)
		{
 		...
 		mprintf("\n");
		mprintf(WHITE,"    Reference plane: Fixing in %s the %d. %s-, the %d. %s- and the %d. %s atom.\n",((CMolecule*)g_oaMolecules[g_iFixMol])->m_sName,g_iFixAtom[0]+1,(const char*)((CAtom*)g_oaAtoms[g_iFixRealAtomType[0]])->m_sName,g_iFixAtom[1]+1,(const char*)((CAtom*)g_oaAtoms[g_iFixRealAtomType[1]])->m_sName,g_iFixAtom[2]+1,(const char*)((CAtom*)g_oaAtoms[g_iFixRealAtomType[2]])->m_sName);
		mprintf("\n");

		if ((((CMolecule*)g_oaMolecules[g_iFixMol])->m_laSingleMolIndex.GetSize() > 1) && g_bAdvanced2) {
			g_iRefMolNum = AskRangeInteger("    Which representative of the reference molecule to use (1-%d)? [1] ",1,((CMolecule*)g_oaMolecules[g_iFixMol])->m_laSingleMolIndex.GetSize(),1,((CMolecule*)g_oaMolecules[g_iFixMol])->m_laSingleMolIndex.GetSize()) - 1;
			mprintf("\n");
		} else
			g_iRefMolNum = 0;
...
_ref2again:
		...
		if ((g_iFixAtomType[0] == g_iFixAtomType[1]) && (g_iFixAtom[0] == g_iFixAtom[1]))
		{
 		...
 		mprintf("\n");
		mprintf(WHITE,"Reference axis: Fixing in %s the %d. %s and the %d. %s atom.\n",((CMolecule*)g_oaMolecules[g_iFixMol])->m_sName,g_iFixAtom[0]+1,(const char*)((CAtom*)g_oaAtoms[g_iFixRealAtomType[0]])->m_sName,g_iFixAtom[1]+1,(const char*)((CAtom*)g_oaAtoms[g_iFixRealAtomType[1]])->m_sName);
		mprintf("\n");
...
_ref1again:
			...
			if (strlen(buf)==0)
			{
				...
				goto _ref1again;
 			mprintf("\n");
			mprintf(WHITE,"Reference atom: Fixing in %s the %d. %s atom.\n",((CMolecule*)g_oaMolecules[g_iFixMol])->m_sName,g_iFixAtom[0]+1,(const char*)((CAtom*)g_oaAtoms[g_iFixRealAtomType[0]])->m_sName);
			mprintf("\n");
		} else
		{
			if (!ParseRefSystem(g_iFixMol,"#2",1))
	...
	if (g_bSaveRefEnv || g_bCutCluster)
	{
		...
		if (g_bSaveRefEnv)
		{
			if (AskYesNo("    Create a temporal development overlay (TDO) plot (y/n)? [no] ",false))
			{
				...
				g_fTDOBleaching = AskRangeFloat("    Enter the TDO bleaching grade (0-1): [0.8] ",0.0,1.0,0.8);
			} else g_bTDO = false;
		}
	}
	...
	if (g_bPlProj || g_bAggregation || g_bNbAnalysis || g_bPlDF || g_bLiDF || g_bRDyn || g_bIRSpec || g_bDipDF || g_bMSD || g_bADF || g_bDDF || g_bVACF || g_bDipACF || g_bRevSDF || g_bSDF || g_bVHDF || g_bRDF || g_bDens || g_bVDF || /*g_bFDF ||*/ g_bCond) // Fuer Verteilungsfunktion: Welches Atom beobachten?
	{
		if (g_bVACF)
		{
			...
			if (g_bPowerSpec)
			{
				g_bGlobalVACF = AskYesNo("    Compute global power spectrum of whole system (y/n)? [yes] ",true);
					...
					if (((CMolecule*)g_oaMolecules[g_iFixMol])->m_laSingleMolIndex.GetSize() > 1)
					{
						...
						if (strlen(buf)==0)
						{
							for (z=0;z<((CMolecule*)g_oaMolecules[o->m_iShowMol])->m_laSingleMolIndex.GetSize();z++)
								o->m_waObsRefList.Add((unsigned short)z);
						} else
						{
				...
				//	mprintf("RefList[0] = %d\n",o->m_waObsRefList[0]);
					...
					if (o->m_iShowMol != -1)
					{
						if (((CMolecule*)g_oaMolecules[o->m_iShowMol])->m_laSingleMolIndex.GetSize() > 1)
						{
							...
							{
								for (z=0;z<((CMolecule*)g_oaMolecules[o->m_iShowMol])->m_laSingleMolIndex.GetSize();z++)
									o->m_waObsShowList.Add((unsigned short)z);
							} else
							{
							...
							if (((CMolecule*)g_oaMolecules[o->m_iShowMol2])->m_laSingleMolIndex.GetSize() > 1)
							{
								...
								if (strlen(buf)==0)
								{
									for (z=0;z<((CMolecule*)g_oaMolecules[o->m_iShowMol2])->m_laSingleMolIndex.GetSize();z++)
										o->m_waObsShow2List.Add((unsigned short)z);
								} else
								{
...
//					case 6: sprintf(buf,"LiDF"); break;
			...
			if (g_bAdvanced2)
				o->m_pCDF->m_iHistogramRes = AskUnsignedInteger("    Please enter CDF histogram resolution (0=no histogram): [0] ",0);
...
_nextcomb:
							...
							{
								eprintf("    Invalid input, %d instead of %d values.\n",tempwa.GetSize(),g_iCDFChannels);
								goto _nextcomb;
							}
							for (z=0;z<g_iCDFChannels;z++)
							{
								...
								{
									eprintf("    Invalid input, channel %d has only %d observations (%d requested).\n",z+1,o->m_pCDF->m_iCombinations[z],tempwa[z]);
									goto _nextcomb;
								}
								tempwa[z]--;
							}
...
_3combdone:
			...
			if (g_bAdvanced2)
			{
				...
				if ((g_iCDFChannels == 2) && (g_iObsChannel[0] == 0) && (g_iObsChannel[1] == 1))
					o->m_pCDF->m_iNormalize = (char)AskRangeInteger("    Normalize uniformly (3), data range (2), integral (1), or do not normalize (0)? [1] ",0,3,1);
						else o->m_pCDF->m_iNormalize = (char)AskRangeInteger("    Normalize data range (2), integral (1), or do not normalize (0)? [1] ",0,2,1);
			...
			} else
			{
				...
				o->m_pCDF->m_fNormValue = 1000000.0;
			}
				...
				if (AskYesNo("    Save temporal development for this observation (y/n)? [no] ",false))
				{
					...
					if (o->m_iShowMol != -1)
					{
						if (((CMolecule*)g_oaMolecules[o->m_iShowMol])->m_laSingleMolIndex.GetSize() > 1)
						{
							...
							{
								for (z=0;z<((CMolecule*)g_oaMolecules[o->m_iShowMol])->m_laSingleMolIndex.GetSize();z++)
									o->m_waSaveShowList.Add((unsigned short)z);
							} else
							{
...
_nobinonly:
				...
				if (g_bCond)
					o->m_bCondDevelopment = AskYesNo("    Save temporal development of this condition (y/n)? [yes] ",true);
				else
					o->m_bCondDevelopment = AskYesNo("    Save temporal development of this condition (y/n)? [no] ",false);
...
//	}
	...
	if ((g_bSDF && g_bAdvanced2) || g_bSaveRefEnv || g_bCutCluster || g_bSaveJustTraj)
	{
		...
		g_bSaveVirtAtoms = AskYesNo("    Write also virtual atoms (center of mass, ...) (y/n)? [no] ",false);
...
_norot:
		...
		if (g_bUnwrap && g_bNPT) {
			mprintf("\n");
			mprintf(RED,"    Warning: ");
			mprintf("Unwrapping does *not* work with variable cell vector (NpT ensemble)!\n");
		...
		if (g_bBoxNonOrtho)
		{
			...
			if (g_bWriteOrtho)
			{
				g_fWriteOrthoFac = AskFloat("      Enter scaling factor (x,y,z coordinates will range from 0 to this value) in pm: [%.4f] ",mypow(g_fBoxVolume,1.0/3.0)*100.0,mypow(g_fBoxVolume,1.0/3.0)*100.0);
				mprintf("\n");
			}
		}
		...
		for (z2=0;z2<g_oaSaveMolecules.GetSize();z2++)
		{
			...
			for (z=0;z<ag->m_baAtomType.GetSize();z++)
			{
				...
				{
					mprintf("%s%d",(const char*)((CAtom*)g_oaAtoms[ag->m_baRealAtomType[z]])->m_sName,((CxIntArray*)ag->m_oaAtoms[z])->GetAt(z3)+1);
...
//		g_bTrajAtomwise = (AskRangeInteger("\n    Save trajectory ordered molecule-wise (0) or atom-wise (1)? [0] ",0,1,0)!=0);
		...
		g_bProcAlternativeLabels = AskYesNo("    Change atom labels of some atoms in output trajectory (y/n)? [no] ",false);
		if (g_bProcAlternativeLabels) {

			try { ag2 = new CAtomGroup(); } catch(...) { ag2 = NULL; }
			if (ag2 == NULL) NewException((double)sizeof(CAtomGroup),__FILE__,__LINE__,__PRETTY_FUNCTION__);

			for (z=0;z<g_iGesVirtAtomCount;z++)
				g_oaProcAlternativeLabels.Add(new CxString());

			for (z2=0;z2<g_oaSaveMolecules.GetSize();z2++)
			{
				ag = (CAtomGroup*)g_oaSaveMolecules[z2];
				mprintf("\n");
				if (!AskYesNo("      - Change some labels in molecule %d (%s) (y/n)? [no] ",false,z2+1,(const char*)ag->m_pMolecule->m_sName))
					continue;
				while (true) {
					mprintf("\n        Which atoms to relabel in %s (e.g. \"C1,C3-5\")? [done] ",(const char*)ag->m_pMolecule->m_sName);
					inpprintf("! Which atoms to relabel in %s (e.g. \"C1,C3-5,H\")? [done]\n",(const char*)ag->m_pMolecule->m_sName);
					myget(&buf);
					if (buf.GetLength() == 0)
						break;
					if (!ag2->ParseAtoms(ag->m_pMolecule,buf))
						continue;
					AskString_ND("        Enter output element label for these atoms: ",&buf);

					for (z=0;z<ag2->m_baAtomType.GetSize();z++)
						for (z3=0;z3<((CxIntArray*)ag2->m_oaAtoms[z])->GetSize();z3++)
							for (z4=0;z4<ag2->m_pMolecule->m_laSingleMolIndex.GetSize();z4++)
								*((CxString*)g_oaProcAlternativeLabels[((CxIntArray*)((CSingleMolecule*)g_oaSingleMolecules[ag2->m_pMolecule->m_laSingleMolIndex[z4]])->m_oaAtomOffset[ag2->m_baAtomType[z]])->GetAt(((CxIntArray*)ag2->m_oaAtoms[z])->GetAt(z3))]) = buf;
				}
			}
			delete ag2;
			...
			for (z2=0;z2<g_oaSaveMolecules.GetSize();z2++)
			{
				...
				for (z=0;z<ag->m_baAtomType.GetSize();z++)
				{
					for (z3=0;z3<((CxIntArray*)ag->m_oaAtoms[z])->GetSize();z3++)
					{
						ti = ((CxIntArray*)((CSingleMolecule*)g_oaSingleMolecules[ag->m_pMolecule->m_laSingleMolIndex[0]])->m_oaAtomOffset[ag->m_baAtomType[z]])->GetAt(((CxIntArray*)ag->m_oaAtoms[z])->GetAt(z3));
						if (((CxString*)g_oaProcAlternativeLabels[ti])->GetLength() != 0) {
							if (!tb) {
								mprintf("    - In %s:\n",ag->m_pMolecule->m_sName);
								tb = true;
							}
							mprintf("       %2s%-3d  ---> %6s\n",(const char*)((CAtom*)g_oaAtoms[ag->m_baRealAtomType[z]])->m_sName,((CxIntArray*)ag->m_oaAtoms[z])->GetAt(z3)+1,(const char*)*((CxString*)g_oaProcAlternativeLabels[ti]));
						}
					}
				}
				if (tb)
		...
		if (g_bAdvanced2) {
			g_bProcWriteComments = AskYesNo("    Write comments (atom label and molecule) behind each line in output trajectory (y/n)? [no] ",false);
			g_bProcDumpAwkScript = AskYesNo("    Create a shell script for resorting other XYZ files (e.g., velocity/force trajectories) (y/n)? [no] ",false);
			g_bProcCellComment = AskYesNo("    Write the cell geometry to the XYZ comment line (y/n)? [no] ",false);
			if (g_bProcCellComment) {
				if (g_bBoxNonOrtho)
					g_bProcCellCommentAngles = AskYesNo("      Write cell edge lengths and angles (y) or only edge lengths (n)? [yes] ",true);
				else
					g_bProcCellCommentAngles = AskYesNo("      Write cell edge lengths and angles (y) or only edge lengths (n)? [no] ",false);
			}
		} else {
		...
		if (g_bAdvanced2) {
			g_bProcSplit = AskYesNo("    Split the trajectory into parts (y/n)? [no] ",false);
			if (g_bProcSplit)
				g_iProcSplitLength = AskUnsignedInteger("    Enter the number of time steps in each part: [1000] ",1000);
		} else {
			g_bProcSplit = false;
			g_iProcSplitLength = -1;
		}

		if (g_bAdvanced2) {
			mprintf("\n");
			g_bProcAddMesh = AskYesNo("    Add a mesh of atoms to the saved trajectory (y/n)? [no] ",false);
			if (g_bProcAddMesh) {
				mprintf("\n");
				if (!g_bPeriodicX || !g_bPeriodicY || !g_bPeriodicZ) {
					eprintf("Error: This only works with a XYZ-periodic cell.\n\n");
					abort();
				}
				mprintf("    The meshes will be periodic with respect to the unit cell entered above.\n\n");

				AskString("    Which atom label to use for the mesh atoms? [Ne] ",&g_sProcAddMeshLabel,"Ne");
				mprintf("\n");

				g_bProcAddMeshZ = AskYesNo("    Add a mesh in the X-Y plane (y/n)? [yes] ",true);
				if (g_bProcAddMeshZ) {
					if (g_bCenterZero)
						tf = -g_fBoxZ/2.0;
					else
						tf = 0;
					g_fProcAddMeshZPos = AskFloat("    At which Z position to add the mesh in the X-Y plane? [%.3f] ",tf,tf);
					mprintf("\n");
				}

				g_bProcAddMeshY = AskYesNo("    Add a mesh in the X-Z plane (y/n)? [yes] ",true);
				if (g_bProcAddMeshY) {
					if (g_bCenterZero)
						tf = -g_fBoxY/2.0;
					else
						tf = 0;
					g_fProcAddMeshYPos = AskFloat("    At which Y position to add the mesh in the X-Z plane? [%.3f] ",tf,tf);
					mprintf("\n");
				}

				g_bProcAddMeshX = AskYesNo("    Add a mesh in the Y-Z plane (y/n)? [yes] ",true);
				if (g_bProcAddMeshX) {
					if (g_bCenterZero)
						tf = -g_fBoxX/2.0;
					else
						tf = 0;
					g_fProcAddMeshXPos = AskFloat("    At which X position to add the mesh in the Y-Z plane? [%.3f] ",tf,tf);
					mprintf("\n");
				}

				g_iProcAddMeshGrid = AskUnsignedInteger("    Enter grid resolution for mesh: [8] ",8);

				g_bProcAddMeshJitter = AskYesNo("    Add random jitter to mesh atoms (e.g. to avoid Voronoi degeneracies, y/n)? [yes] ",true);

				if (g_bProcAddMeshJitter)
					g_fProcAddMeshJitter = AskFloat("    Enter the amount of random jitter on mesh atom coordinates (in pm): [1.0] ",1.0);
			}
		} else
			g_bProcAddMesh = false;
	...
	if (g_bUnwrap)
	{
		...
		for (z=0;z<g_vaUnwrapArray.GetSize();z++)
			g_vaUnwrapArray[z] = CxDVector3(0);
	}
...
//		mprintf("\n    Not wrapping molecules into the box.\n");
	...
	if (g_bMSD || g_bPairMSD || g_bSaveJustTraj || g_bACF || (g_bIRSpec && g_bDipoleRefFixed) || g_bVHDF || g_bPDF || g_bFixedPlProj)
	{
		if (g_bUnwrap || g_bBoxNonOrtho)
			g_bRemoveCOM = AskYesNo("    Remove center of mass movement of the box (y/n)? [no] ",false);
		else
			g_bRemoveCOM = AskYesNo("    Remove center of mass movement of the box (y/n)? [yes] ",true);
		mprintf("\n");
	} else
	{
	...
	if (g_bRemoveCOM && g_bAdvanced2) {

		if (!AskYesNo("    Fix com of whole system (y) or a specified center/atom (n)? [yes] ",true)) {

			if (g_oaMolecules.GetSize() > 1)
				ti = AskRangeInteger_ND("    Take atom from which molecule type (1-%d)? ",1,g_oaMolecules.GetSize(),g_oaMolecules.GetSize()) - 1;
			else
				ti = 0;

			if (((CMolecule*)g_oaMolecules[ti])->m_laSingleMolIndex.GetSize() > 1)
				ti2 = AskRangeInteger_ND("    Take atom from which molecule of %s (1-%d)? ",1,((CMolecule*)g_oaMolecules[ti])->m_laSingleMolIndex.GetSize(),((CMolecule*)g_oaMolecules[ti])->m_sName,((CMolecule*)g_oaMolecules[ti])->m_laSingleMolIndex.GetSize()) - 1;
			else
				ti2 = 0;

			AskString("    Which atom to take from %s %d? [#2] ",&buf,"#2",((CMolecule*)g_oaMolecules[ti])->m_sName,ti2+1);
			ParseAtom((const char*)buf,ti,ty,rty,atom);
			g_iRemoveCOMFixAtom = ((CxIntArray*)((CSingleMolecule*)g_oaSingleMolecules[((CMolecule*)g_oaMolecules[ti])->m_laSingleMolIndex[ti2]])->m_oaAtomOffset[ty])->GetAt(atom);
			mprintf("    The atom has offset %d.\n\n",g_iRemoveCOMFixAtom);
		} else
			g_iRemoveCOMFixAtom = -1;
	}

	if (g_bUseVelocities && (g_fTimestepLength > 1.0) && !g_bROA)
	{
		mprintf(RED,"    Warning: ");
...
//					strcat(buf," [1] ");
	...
	if (g_bAdvanced2)
	{
		if (AskYesNo("    Perform a multi-interval analysis (y/n)? [no] ",false))
		{
			mprintf("\n");
			mprintf("    Warning:");
			mprintf(" The multi-inverval analysis is a relic from the early times of TRAVIS.\n");
			mprintf("             Many of the new / updated functions are no longer compatible. However,\n");
			mprintf("             some functions may still work. Please be very careful with the results!\n\n");
			mprintf("             A workaround would be to perform several TRAVIS runs instead, and specify\n");
			mprintf("             the corresponding starting time steps / step counts. This can be nicely\n");
			mprintf("             automatized by a small shell script.\n\n");
			if (!AskYesNo("    Acknowledged (y/n) [yes] ",true))
				return false;
			mprintf("\n");
...
_multi:
	if (g_iTrajSteps != -1) {
		...
		if (g_iMaxStep == -1)
			mprintf(YELLOW,"\n    Using %d time steps: Every %d%s within range %d - approx. %d.\n",(g_iTrajSteps-g_iBeginStep)/g_iStride,g_iStride,(g_iStride==1)?"st":((g_iStride==2)?"nd":((g_iStride==3)?"rd":"th")),g_iBeginStep+1,g_iTrajSteps);
		else
			mprintf(YELLOW,"\n    Using %d time steps: Every %d%s within range %d - %d.\n",g_iMaxStep/g_iStride,g_iStride,(g_iStride==1)?"st":((g_iStride==2)?"nd":((g_iStride==3)?"rd":"th")),g_iBeginStep+1,g_iBeginStep+g_iMaxStep);
		...
		if (g_iMaxStep != -1)
			...
			mprintf(YELLOW,"\n    Using every %d%s time step beginning from step %d.\n",g_iStride,(g_iStride==1)?"st":((g_iStride==2)?"nd":((g_iStride==3)?"rd":"th")),g_iBeginStep+1);
	}
