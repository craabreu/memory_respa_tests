			...
			if (g_bVDF && (o->m_pVDF[z0] != NULL))
			{
				...
				if (o->m_pVDF[z0]->m_bSplitCart) {
					for (z2=0;z2<6;z2++) {
						...
						switch(z2) {
							case 0: o->m_pVDF[z0]->m_pVDFSplit[z2]->SetLabelX("Velocity X Projection [pm/ps]"); break;
							case 1: o->m_pVDF[z0]->m_pVDFSplit[z2]->SetLabelX("Velocity Y Projection [pm/ps]"); break;
							case 2: o->m_pVDF[z0]->m_pVDFSplit[z2]->SetLabelX("Velocity Z Projection [pm/ps]"); break;
							case 3: o->m_pVDF[z0]->m_pVDFSplit[z2]->SetLabelX("Velocity XY Projection [pm/ps]"); break;
							case 4: o->m_pVDF[z0]->m_pVDFSplit[z2]->SetLabelX("Velocity XZ Projection [pm/ps]"); break;
							case 5: o->m_pVDF[z0]->m_pVDFSplit[z2]->SetLabelX("Velocity YZ Projection [pm/ps]"); break;
						}
...
//		sprintf(buf,"traj_out.xyz");
		...
		if (g_bProcDumpAwkScript) {
			...
			mfprintf(a,"  print \"Script created by TRAVIS on %s\" > \"/dev/stderr\"\n",(const char*)obuf);
							...
							for (z3=0;z3<g_oaSaveMolecules.GetSize();z3++)
							{
								...
								for (z6=0;z6<atgr->m_pMolecule->m_laSingleMolIndex.GetSize();z6++)
								{
									...
									//			mprintf("\nUnwrap: (%G|%G|%G) <-- (%G|%G|%G)",vec0[0],vec0[1],vec0[2],g_vaUnwrapArray[atgr->m_pMolecule->m_laSingleMolIndex[z6]][0],g_vaUnwrapArray[atgr->m_pMolecule->m_laSingleMolIndex[z6]][1],g_vaUnwrapArray[atgr->m_pMolecule->m_laSingleMolIndex[z6]][2]);
											...
											{
												if (g_bSaveVirtAtoms)
													mfprintf(g_fSaveJustTraj,"%-4s %12.8f  %12.8f  %12.8f",((CVirtualAtom*)g_oaVirtualAtoms[atgr->m_pMolecule->m_laVirtualAtoms[tla->GetAt(z5)]])->m_sLabel,vec0[0]/100.0,vec0[1]/100.0,vec0[2]/100.0);
											} else {
												cp = ((CAtom*)g_oaAtoms[atgr->m_baRealAtomType[z4]])->m_sName;
												if (g_bProcAlternativeLabels)
													if (((CxString*)g_oaProcAlternativeLabels[((CxIntArray*)((CSingleMolecule*)g_oaSingleMolecules[atgr->m_pMolecule->m_laSingleMolIndex[z6]])->m_oaAtomOffset[atgr->m_baAtomType[z4]])->GetAt(tla->GetAt(z5))])->GetLength() != 0)
					...
					if (g_bPeriodicX && g_bPeriodicY && g_bPeriodicZ)
					{
						...
						} else
						{
							if (g_bBoxNonOrtho) {
								...
								o->m_pLiDF[zr]->m_pLiDF->MultiplyBin(g_fBoxVolume / (4.0/3.0*Pi) / g_iSteps / ((CMolecule*)g_oaMolecules[g_iFixMol])->m_laSingleMolIndex.GetSize() / o->m_pLiDF[zr]->m_iShowAtomGes / ((CMolecule*)g_oaMolecules[o->m_iShowMol])->m_laSingleMolIndex.GetSize() / o->m_pLiDF[zr]->m_iRefAtomGes);
							} else {
								o->m_pLiDF[zr]->m_pLiDF->Integrate(true,1.0 / g_iSteps / (double)((CMolecule*)g_oaMolecules[g_iFixMol])->m_laSingleMolIndex.GetSize());
				...
				} else
				{
					...
					if (o->m_bOthers && o->m_pRDF[zr]->m_bRadialCorrect)
					{
						if (o->m_pRDF[zr]->m_bProbDens)
						{
							...
							{
								o->m_pRDF[zr]->m_pRDF->Integrate(true,1.0 / (double)((CMolecule*)g_oaMolecules[g_iFixMol])->m_laSingleMolIndex.GetSize() / o->m_pRDF[zr]->m_iRefAtomGes);
