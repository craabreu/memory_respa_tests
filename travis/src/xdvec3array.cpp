...
CxDVec3Array::CxDVec3Array(const CxDVec3Array &o) : CxObject()
{
	BXIN;
#ifdef DEBUG_CDVEC3ARRAY
	...
	if ((o.m_iMaxSize != 0) && (o.m_pData != NULL)) {
		...
		if (o.m_pData != NULL) {
			...
			for (z=0;z<(int)m_iSize;z++)
				m_pData[z] = CxDVector3(o.m_pData[z]);
		}
...
//	CxDVector3 **temp;
	...
	if (i <= m_iSize)
	{
		eprintf("CxDVec3Array::SetMaxSize(%lu): Size %lu > %lu.\n",i,m_iSize,i);
		return;
	}
//	temp = new CxDVector3*[i];
...
/*	for (z=0;z<(int)pos;z++)
	...
	if (m_pData != NULL) {
		...
		for (z=0;z<(int)pos;z++)
			temp[z] = CxDVector3(m_pData[z]);
		for (;z<(int)m_iSize-(int)count;z++)
			temp[z] = CxDVector3(m_pData[z+count]);
		delete[] m_pData;
	}
	m_pData = temp;
	m_iSize -= count;
