...
public:
	...
	CBQBReadCache(CBQBInterface &i) : m_IF(i), m_iHistoryDepth(0), m_iCachePos(0), m_iReadPos(0),
		m_fRef(NULL), m_fInput(NULL), m_bStartLock(false) {
	}
	...
	void SetStartLock() {
		if (m_bStartLock) {
			m_IF.eprintf("CCCReadCache::SetStartLock(): Error: Start lock already set.\n");
			abort();
		}
		m_bStartLock = true;
	}
	...
	void LiftStartLock() {
		if (!m_bStartLock) {
			m_IF.eprintf("CCCReadCache::LiftStartLock(): Error: Start lock not in place.\n");
			abort();
		}
		m_bStartLock = false;
	}
	...
	const FT* GetFrameHistory(int i) const {
		if (i > m_iHistoryDepth) {
			m_IF.eprintf("CCCReadCache::GetFrameHistory(): Error: Index out of range (%d/%d).\n",i,m_iHistoryDepth);
			abort();
		}
		int t = m_iReadPos - i;
		if (t < 0)
	...
	const FT* GetNextFrame() {
		if (m_iReadPos == m_iCachePos) {
			...
			if (m_iCachePos >= (int)m_oaFrames.size()) {
				if (m_bStartLock) {
					m_IF.eprintf("CCCReadCache::GetNextFrame(): Error: Violation of start lock.\n");
					abort();
				}
				m_iCachePos = 0;
			}
	...
	bool CacheSteps(int i, bool progress) {
		...
		for (int z=0;z<i;z++) {
			...
			if (m_iCachePos >= (int)m_oaFrames.size()) {
				if (m_bStartLock) {
					m_IF.eprintf("CCCReadCache::CacheSteps(): Error: Violation of start lock.\n");
					abort();
				}
				m_iCachePos = 0;
			}
	...
	void RewindReadPos() {
		if (!m_bStartLock) {
			m_IF.eprintf("CCCReadCache::RewindReadPos(): Error: Requires start lock.\n");
			abort();
		}
		m_iReadPos = -1;
	}
	...
	bool FileOpenRead(const char *inp, const char *ref = NULL) {
		...
		if (m_fInput == NULL) {
			m_IF.eprintf("CCCReadCache::FileOpenRead(): Error: Could not open input file for reading.\n");
			return false;
		}
		if (ref != NULL) {
			m_fRef = fopen(inp,"wb");
			if (m_fRef == NULL) {
				m_IF.eprintf("CCCReadCache::FileOpenRead(): Error: Could not open reference file for writing.\n");
				fclose(m_fInput);
				m_fInput = NULL;
				return false;
			}
...
public:
	...
	bool SkipOneStep() {
		return CBQBAtomSet().SkipXYZ(m_fInput);
	}
...
private:
	bool CacheOneStep() {
		...
		m_oaFrames[m_iCachePos] = new CBQBAtomSet();
		if (m_fInput == NULL) {
...
public:
	...
	void ExportCellInfo(CBQBCellInfo *ci, CBQBBitSet *bs);

	bool ImportCellInfo(CBQBCellInfo *ci, CBQBBitSet *bs);
	...
	struct SSortAtomOrd {
		...
		SSortAtomOrd(const CBQBEngine &e) : m_E(e) {
		}
