...
double C2DF::PPMBin()
{
	...
	for (z=0;z<m_iRes[0]*m_iRes[1];z++)
	{
		m_pBin[z] *= 1000000.0 / (double)g_iSteps / ((CMolecule*)g_oaMolecules[g_iFixMol])->m_laSingleMolIndex.GetSize();
		if (f < m_pBin[z])
			f = m_pBin[z];
	}
	BTOUT;
...
void C2DF::WriteGraceBunch(int channel, int graphs, double fac, const char *prefix, const char *name, const char *suffix)
{
	BTIN;
	int x, y, t;
//	char buf[32768];
...
//	char buf[32768];
	...
	comb->SetViewport(0.15,0.8,0.8,0.95);
	...
	comb->AddGraph();
	comb->SetViewport(0.8, 0.15, 0.95, 0.8);
	comb->CurrentGraph()->m_bTicks = false;
	comb->CurrentGraph()->m_bTickLabels = false;
	comb->CurrentGraph()->m_fFrameWidth = 0.5;
	...
	comb->AddGraph();
	comb->SetViewport(0.15, 0.15, 0.8, 0.8);
...
void C2DF::Mirror(double plane, int channel)
{
	int x, y, t;
	double *tbin, p;
