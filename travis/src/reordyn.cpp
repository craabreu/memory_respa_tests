...
_ax6:		mprintf("      Velocity vector of which atoms to use (e.g. C7)? [#2] ");
		...
		if (strlen(buf)==0)
		{
			...
			{
				eprintf("CReorDyn::Parse(): Internal error (1).\n");
...
_ax7:		mprintf("      Force vector of which atoms to use (e.g. C7)? [#2] ");
		...
		if (strlen(buf)==0)
		{
			if (!ag->ParseAtoms((CMolecule*)g_oaMolecules[m_iShowMol],"#2"))
			{
				eprintf("CReorDyn::Parse(): Internal Error (2).\n");
				inpprintf("! CReorDyn::Parse(): Internal Error.\n");
				abort();
			}
		} else if (!ag->ParseAtoms((CMolecule*)g_oaMolecules[m_iShowMol],buf))
...
/*	if (g_bRDynCacheMode)
	{
		tf = g_iTrajSteps*m_iCombinations*((CMolecule*)g_oaMolecules[m_iShowMol])->m_waSingleMolIndex.GetSize()*3.0f*sizeof(double)/1024.0f/1024.0f;
//		if (tf >= 10.0f)
...
//		if (tf >= 10.0f)
	...
	mprintf("\n");
	m_bLegendre2 = AskYesNo("    Perform autocorrelation of 2nd Legendre polynomial (y/n)? [no] ",false);
...
//	char buf[256];
	CxString buf;
	CxDoubleArray *ptfa, *ptfab, *ptfa2, *ptfa2b, *ptfa3;
	CAutoCorrelation *ac;
	...
	if (g_bRDynCacheMode)
	{
			...
			switch(m_pACF->m_iDerivative)
			{
				...
				case 1:
					...
					for (z2=0;z2<m_iMolecules*m_iCombinations;z2++)
					{
						if (fmod(z2,tfs) < 1.0)
							mprintf(WHITE,"#");
						ptfa = (CxDoubleArray*)m_oaCache[z2];
		...
		try { ptfa2 = new CxDoubleArray("CReorDyn::Finish():ptfa2"); } catch(...) { ptfa2 = NULL; }
		...
		try { ptfa3 = new CxDoubleArray("CReorDyn::Finish():ptfa3"); } catch(...) { ptfa3 = NULL; }
...
/*************************************************************************/
		...
		try { ptfa2b = new CxDoubleArray("CReorDyn::Finish():ptfa2b"); } catch(...) { ptfa2b = NULL; }
		...
		for (z2=0;z2<m_iMolecules*m_iCombinations;z2++)
		{
			...
			for (z4=0;z4<m_iMolecules*m_iCombinations;z4++)
			{
				...
				ptfab = (CxDoubleArray*)m_oaCache[z4];
...
//				sprintf(buf,"acf_%s%s%s.w.csv",s,m_sName,multibuf);
		...
		delete g_pFFT;
		m_pACF->m_pSpectrum->SetMaxRWL(1E7/299.792/g_fTimestepLength/g_iStride);
