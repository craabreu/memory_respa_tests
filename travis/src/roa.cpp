...
_again2:
					...
					if (mystricmp((const char*)buf,"*") == 0) {
						...
						for (z2=0;z2<tia.GetSize();z2++)
							obs->m_iaMolSelection[z].push_back(tia[z2]);
					}
				}
			}
			mprintf("\n");
...
_resagain:
		if (g_iTrajSteps != -1) {
			...
			if (obs->m_iDepth > ti)
				obs->m_iDepth = ti;
			obs->m_iDepth = AskUnsignedInteger("    Enter the resolution (=depth) of the correlation functions (in time steps): [%d] ",
				obs->m_iDepth, obs->m_iDepth);
		} else
			obs->m_iDepth = AskUnsignedInteger("    Enter the resolution (=depth) of the correlation functions (in time steps): [512] ",
				512);
		...
		if(g_bAdvanced2) {
			...
			if (obs->m_iWindowFunction == 1) {
				...
				obs->m_iWindowFunctionParameter = AskUnsignedInteger("    Parameter \"a\" (in time steps): [%d] ",
		...
		double specLimit = AskRangeFloat("\n    Calculate spectrum up to which wave number (cm^-1)? [%.2f cm^-1] ",
			0, possibleRange, (possibleRange < 5000.0) ? possibleRange : 5000.0, (possibleRange < 5000.0) ? possibleRange : 5000.0);
		...
		for (z=0;z<(int)obs->m_iaMolSelection.size();z++) {
			...
			if ((int)obs->m_iaMolSelection[z].size() != ((CMolecule*)g_oaMolecules[z])->m_laSingleMolIndex.GetSize()) {
				for (z2=0;z2<(int)obs->m_iaMolSelection[z].size();z2++) {
					buf.sprintf("_%d",obs->m_iaMolSelection[z][z2]);
...
bool CROAEngine::CheckTrajColumns(CROATrajectory *tr) {
	...
	if (!tr->m_bBQB) {
		eprintf("Error: Trajectory is not in BQB format.\n");
		return false;
	}

	if (!tr->m_pBQB->ReadFrame()) {
		eprintf("Error while reading first BQB frame.\n");
		return false;
	}
...
bool CROAEngine::Parse() {
	...
	if (!g_bStrideParsed) {
		...
		if ((m_iMode == ROA_MODE_GATHER) || (m_iMode == ROA_MODE_COMBINED)) {
	...
	//		ti = (int)floor(4.01/g_fTimestepLength);
			ti = 1;
			g_iStride = AskUnsignedInteger("    Use every n-th time step from the trajectory? [%d] ",ti,ti);
		} else
...
/*	_again0:
		...
		if ((mystricmp(GetFileExtension((const char*)g_sInputTraj),"bqb") != 0) &&
			...
			eprintf("Error: File extension of data files should be .bqb, .emp, or .blist\n");
			if (m_bPola)
	...
	_outagain:
			...
			for (z=0;z<g_oaSingleMolecules.GetSize();z++) {
				...
				for (z2=0;z2<m_iStepsProcessed-4;z2++) {
					...
					if ((rn->m_faCharge[z2+2] < tfmi-tfout*(tfma-tfmi)) || (rn->m_faCharge[z2+2] > tfma+tfout*(tfma-tfmi)))
						m_iaOutliers[0][z2]++;
				}
				...
				for (z2=0;z2<m_iStepsProcessed-4;z2++) {
					...
					for (z3=0;z3<3;z3++) {
						...
						if ((rn->m_vaElDip[z2+2][z3] < tfmi-tfout*(tfma-tfmi)) || (rn->m_vaElDip[z2+2][z3] > tfma+tfout*(tfma-tfmi)))
							tb = true;
					}
					if (tb)
						m_iaOutliers[1][z2]++;
				...
				for (z2=0;z2<m_iStepsProcessed-4;z2++) {
					...
					for (z3=0;z3<9;z3++) {
						...
						if ((rn->m_maElQuad[z2+2][z3] < tfmi-tfout*(tfma-tfmi)) || (rn->m_maElQuad[z2+2][z3] > tfma+tfout*(tfma-tfmi)))
							tb = true;
					}
					if (tb)
						m_iaOutliers[2][z2]++;
				...
				for (z2=0;z2<m_iStepsProcessed-4;z2++) {
					...
					for (z3=0;z3<3;z3++) {
						...
						if ((rn->m_vaElCurr[z2+2][z3] < tfmi-2.0*tfout*(tfma-tfmi)) || (rn->m_vaElCurr[z2+2][z3] > tfma+2.0*tfout*(tfma-tfmi)))
							tb = true;
					}
					if (tb)
						m_iaOutliers[3][z2]++;
				...
				for (z2=0;z2<m_iStepsProcessed-4;z2++) {
					...
					for (z3=0;z3<3;z3++) {
						...
						if ((rn->m_vaMagDip[z2+2][z3] < tfmi-2.0*tfout*(tfma-tfmi)) || (rn->m_vaMagDip[z2+2][z3] > tfma+2.0*tfout*(tfma-tfmi)))
							tb = true;
					}
					if (tb)
						m_iaOutliers[4][z2]++;
				...
				for (z2=0;z2<m_iStepsProcessed-4;z2++) {
					...
					for (z3=0;z3<9;z3++) {
						...
						if ((rn->m_maPolElDip[z2+2][z3] < tfmi-tfout*(tfma-tfmi)) || (rn->m_maPolElDip[z2+2][z3] > tfma+tfout*(tfma-tfmi)))
							tb = true;
					}
					if (tb)
						m_iaOutliers[5][z2]++;
				...
				for (z2=0;z2<m_iStepsProcessed-4;z2++) {
					...
					for (z3=0;z3<27;z3++) {
						...
						if ((rn->m_taPolElQuad[z2+2][z3] < tfmi-tfout*(tfma-tfmi)) || (rn->m_taPolElQuad[z2+2][z3] > tfma+tfout*(tfma-tfmi)))
							tb = true;
					}
					if (tb)
						m_iaOutliers[6][z2]++;
	...
	_outagain2:
			...
			for (z=0;z<m_iStepsProcessed-4;z++) {
				...
				if (tb2) {
					...
					if ((m_iaOutliers[3][z] < g_oaSingleMolecules.GetSize()/4) &&
						(m_iaOutliers[4][z] < g_oaSingleMolecules.GetSize()/4) &&
						(m_iaOutliers[7][z] < g_oaSingleMolecules.GetSize()/4))
						continue;
					...
					if ((m_iaOutliers[0][z] < g_oaSingleMolecules.GetSize()/4) &&
						(m_iaOutliers[1][z] < g_oaSingleMolecules.GetSize()/4) &&
						(m_iaOutliers[2][z] < g_oaSingleMolecules.GetSize()/4) &&
						(m_iaOutliers[5][z] < g_oaSingleMolecules.GetSize()/4) &&
						(m_iaOutliers[6][z] < g_oaSingleMolecules.GetSize()/4))
						continue;
...
void CROAEngine::ComputeACF(CROAObservation *obs) {
	...
	for (z=0;z<g_oaMolecules.GetSize();z++) {
		...
		for (z2=0;z2<(int)obs->m_iaMolSelection[z].size();z2++) {
			...
			imol = ((CMolecule*)g_oaMolecules[z])->m_laSingleMolIndex[z2];
			...
			if (obs->m_bCrossCorrelation) {
					...
					for (z3=0;z3<g_oaMolecules.GetSize();z3++) {
						for (z4=0;z4<(int)obs->m_iaMolSelection[z3].size();z4++) {
							if ((z == z3) && (z2 == z4))
								continue;
							imol2 = ((CMolecule*)g_oaMolecules[z3])->m_laSingleMolIndex[z4];
...
void CROAEngine::ComputeSpectrum(CROAObservation *obs) {
	...
	/*********************************************************************************************************************/
		...
		ComputeSpectrum(obs,obs->m_faACF,obs->m_faSpectrum,"_re",false);
		ComputeSpectrum(obs,obs->m_faACF,obs->m_faSpectrum,"_im",true);

		if (obs->m_bSingleACFs) {
			for (z=0;z<27;z++) {
				buf.sprintf("_single_%c%c_%c_re",'x'+(z/9),'x'+((z/3)%3),'x'+(z%3));
				ComputeSpectrum(obs,obs->m_faaSingleACFs[z],obs->m_faSpectrum,(const char*)buf,false);
				buf.sprintf("_single_%c%c_%c_im",'x'+(z/9),'x'+((z/3)%3),'x'+(z%3));
				ComputeSpectrum(obs,obs->m_faaSingleACFs[z],obs->m_faSpectrum,(const char*)buf,true);
			}
		}
