			...
			for (iy=0;iy<m_iSRange[1];iy++) {
				...
				for (ix=0;ix<m_iSRange[0];ix++) {
					...
					if (m_IF.IsPL(BQB_PL_VERBOSE)) {
						...
						m_IF.printf("  %lu different X values:",setx.size());
						for (setit=setx.begin();setit!=setx.end();++setit)
							m_IF.printf(" %d",*setit);
						m_IF.printf("\n");

						m_IF.printf("  %lu different Y values:",sety.size());
						for (setit=sety.begin();setit!=sety.end();++setit)
							m_IF.printf(" %d",*setit);
						m_IF.printf("\n");

						m_IF.printf("  %lu different Z values:",setz.size());
						for (setit=setz.begin();setit!=setz.end();++setit)
							m_IF.printf(" %d",*setit);
						m_IF.printf("\n");

						m_IF.printf("  %lu different T values:",sett.size());
