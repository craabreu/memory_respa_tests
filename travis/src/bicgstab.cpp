...
// 			}
	...
	if (j != numEntries) {
		eprintf("CCurrentPDEDiscretizer::discretize(): Error in PDE discretizer.\n");
		abort();
	}
 ...
 * notice are retained.                                    *
 ***********************************************************/
bool CCurrentPDESolver::bicgstabl(const int l, const CSparseMatrix *pdeMatrix, double *solution, const double *rightHandSide, const int maxIter, double *thresh, FILE *infoFile, bool fastmode) {
	double **r;
	...
	double retVal;
	double *bestsol, bestresi;

	if (fastmode) {
		try { bestsol = new double[pdeMatrix->m_size]; } catch(...) { bestsol = NULL; }
		if (bestsol == NULL) NewException((double)pdeMatrix->m_size * sizeof(double), __FILE__, __LINE__, __PRETTY_FUNCTION__);
		bestresi = 1.0e30;
	} else
		bestsol = NULL;
	...
	for (i=0;i<l+1;i++)
		for (int k=0;k<pdeMatrix->m_size;k++)
			u[i][k] = 0;
...
// 	mprintf(GREEN, "%#.10g\n", r[0][10]);
	...
	while (numIter < maxIter && resNorm > *thresh) {
		...
		for (i = 0; i < l; i++) {
			...
			if (rho0 == 0.0) {
				eprintf("CCurrentPDESolver::BiCGSTABl(): Error: Rho0 is zero!\n");
				abort();
			}
			...
			if (sigma == 0.0) {
				eprintf("CCurrentPDESolver::BiCGSTABl(): Error: Sigma is zero!\n");
				abort();
			}
			alpha = rho1 / sigma;
			for (j = 0; j < pdeMatrix->m_size; j++)
...
// 			mprintf(GREEN, "%#.14g %#.10g\n", resNorm, r[0][1942857]);
		...
		if (fastmode && (resNorm < bestresi)) {

			bestresi = resNorm;

			for (i = 0; i < pdeMatrix->m_size; i++)
				bestsol[i] = solution[i] + x[i];
		}
	}

	if ((resNorm <= *thresh) && (numIter == maxIter)) // Detect convergence in last step
		numIter--;
...
// 	mprintf(GREEN, "%#.10g\n", matrix->m_val[13600000]);
	...
	for (i = 0; i < matrix->m_size; i++) {
		...
		if (diagInd[i] == -1) {
			eprintf("CILUPreconditioner::precondition(): Error: Missing diagonal element in row %d!\n", i + 1);
			abort();
		}
	}
