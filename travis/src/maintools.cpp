...
//	mprintf("");
	mprintf("     Copyright (c) Martin Brehm      (2009-2019), University of Halle (Saale)\n");
...
//	mprintf("     Open-source freeware; Licensed under the GNU General Public License v3.\n\n");
	mprintf("     Please cite: ");
	mprintf(WHITE,"M. Brehm, B. Kirchner: J. Chem. Inf. Model. 2011, 51 (8), pp 2007-2023.\n\n");
...
/*	mprintf(WHITE,"  #  ");
	...
	cd = ca+3;
	mprintf(WHITE,"  #  ");
	mprintf("Machine: int=%lub, long=%lub, addr=%lub, 0xA0B0C0D0=%02X,%02X,%02X,%02X.\n",sizeof(int),sizeof(long),sizeof(void*),*ca,*cb,*cc,*cd);
...
_unk:
	eprintf("Could not determine trajectory file format (%s).\n\n",p);
...
void WriteCredits()
{
	...
	if (g_bUseBQB) {
		mprintf(RED,"  * ");
		mprintf("\"An Efficient Lossless Compression Algorithm for Trajectories of Atom Positions and Volumetric Data\",\n    M. Brehm, M. Thomas; J. Chem. Inf. Model. 2018, 58 (10), pp 2092-2107.\n\n");
	}
	if (g_bIRSpec || g_bPowerSpec || g_bRaman || g_bPower || g_bIR || g_bROA)
	{
		mprintf(RED,"  * ");
	...
	if (g_bROA) {
		if (g_pROAEngine->m_bROA) {
			mprintf(RED,"  * ");
			mprintf("\"Computing Bulk Phase Raman Optical Activity Spectra from ab initio Molecular Dynamics Simulations\",\n    M. Brehm, M. Thomas; J. Phys. Chem. Lett. 2017, 8 (14), pp 3409-3414.\n\n");
		}
	}
	if (g_bHBond || g_bIonPair)
	{
		mprintf(RED,"  * ");
		mprintf("\"Structure and lifetimes in ionic liquids and their mixtures\",\n    S. Gehrke, M. von Domaros, R. Clark, O. Holloczki, M. Brehm, T. Welton, A. Luzar, B. Kirchner;\n    Faraday discuss. 2018, 206, pp 219-245.\n\n");
	}
	if (g_bOrder) {
		mprintf(RED,"  * ");
		mprintf("\"Influence of Small Fluorophilic and Lipophilic Organic Molecules on Dipalmitoylphosphatidylcholine Bilayers\",\n    M. Brehm, G. Saddiq, T. Watermann, D. Sebastiani;\n    J. Phys. Chem. B 2017, 121 (35), 8311-8321.\n");
...
//	mprintf("    Computing inverse of transformation matrix...\n\n");
	...
	if (!g_mBoxToOrtho.Invert()) {
		eprintf("ExtractXYZCellGeometry(): Error: Encountered singular cell matrix (cell volume is zero).\n");
		abort();
	}
