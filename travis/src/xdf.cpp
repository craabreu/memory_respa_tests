...
void CXDF::InitDeriv()
{
	...
	if (m_bACF)
	{
		if (m_bSelf)
		{
			try { m_pfaACFBuffer = new CxDoubleArray*[((CMolecule*)g_oaMolecules[g_iFixMol])->m_laSingleMolIndex.GetSize()*m_iCombinations]; } catch(...) { m_pfaACFBuffer = NULL; }
			if (m_pfaACFBuffer == NULL) NewException((double)((CMolecule*)g_oaMolecules[g_iFixMol])->m_laSingleMolIndex.GetSize()*m_iCombinations*sizeof(CxDoubleArray*),__FILE__,__LINE__,__PRETTY_FUNCTION__);
			
			for (z2=0;z2<((CMolecule*)g_oaMolecules[g_iFixMol])->m_laSingleMolIndex.GetSize()*m_iCombinations;z2++)
			{
				try { ptfa = new CxDoubleArray("CXDF::InitDeriv():ptfa"); } catch(...) { ptfa = NULL; }
		...
		{
			try { m_pfaACFBuffer = new CxDoubleArray*[((CMolecule*)g_oaMolecules[g_iFixMol])->m_laSingleMolIndex.GetSize()*((CMolecule*)g_oaMolecules[m_iShowMol])->m_laSingleMolIndex.GetSize()*m_iCombinations]; } catch(...) { m_pfaACFBuffer = NULL; }
			if (m_pfaACFBuffer == NULL) NewException((double)((CMolecule*)g_oaMolecules[g_iFixMol])->m_laSingleMolIndex.GetSize()*((CMolecule*)g_oaMolecules[m_iShowMol])->m_laSingleMolIndex.GetSize()*m_iCombinations*sizeof(CxDoubleArray*),__FILE__,__LINE__,__PRETTY_FUNCTION__);
...
void CXDF::Autocorrelate()
{
	...
	try { ptfa = new CxDoubleArray("CXDF::Autocorrelate():ptfa"); } catch(...) { ptfa = NULL; }
	if (ptfa == NULL) NewException((double)sizeof(CxDoubleArray),__FILE__,__LINE__,__PRETTY_FUNCTION__);
