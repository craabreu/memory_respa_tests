...
{ UNUSED(nBytes); UNUSED(fold);
  ...
  if (diff < 32)
	...
	x->ieee.exponent = 1;
#ifdef HILBERT_CONFORMING
	  x->ieee.mantissa0 = (unsigned)(-((signed)normalized));
	  x->ieee.mantissa1 = (unsigned)(-((signed)normalized));
#else
  ...
  while (y-- > 0)
	   ...
	   dimensions don't matter. */
	{
	  ...
	  if (cornerdiff && digit)
	    ...
	    {
#ifdef HILBERT_CONFORMING
		  p_firstSeparator = digit & ((unsigned long)(-((signed long)digit)));
#else
      ...
      if (index++ != ndOnes)
        {
          rb = b;
#ifdef HILBERT_CONFORMING
		  rd = index & ((unsigned long)(-((signed long)index)));
#else
		  rd = index & -index;
#endif
...
#define maxDim (8*sizeof(bitmask_t))
  ...
  for (;;)
    {
      m_IF.printf( "Enter nDims, nBits, nPrints, orderCheck: " );
      scanf( "%d", &nDims);
      if ( nDims == 0 )
        break;
      ...
      if (nDims*nBits > 8*sizeof(r))
        {
          m_IF.printf("Product of nDims and nBits not exceed %d.\n", 8*sizeof(r));
          break;
        }

      if (nBits == 0)
        {
          m_IF.printf("nBits must be positive.\n");
          break;
        }
	  ...
	  int miscount = 0;
          ...
          m_IF.printf("%d: ", (unsigned)r);
          for (i = 0; i < nDims; ++i)
	    {
	      int diff = (int)(coord[i] - coordPrev[i]);
	      miscount += abs(diff);
	      coordPrev[i] = coord[i];
	      m_IF.printf(" %d", (unsigned)coord[i]);
	    }
	  if (r > 0 && miscount != 1)
	    m_IF.printf(".....error");
	  m_IF.printf("\n");
	  ...
	  for (r1 = 0; r1 < r; ++r1 )
	      ...
	      if (ans != 1)
		{
		  ...
		  m_IF.printf( "cmp r = 0x%0*x; r1 = 0x%0*x, ans = %2d\n",
			  width, (unsigned)r,
	  ...
	  if (hilbert_cmp( nDims, sizeof(coord[0]), nBits, coord, coord1) != 0)
	    m_IF.printf( "cmp r = 0x%0*x; r1 = 0x%0*x\n", (nDims*nBits+3)/4, (unsigned)r,
...
int main()
{
  ...
  m_IF.printf("Enter one corner (%d coordinates): ", nDims);
  for (k = 0; k < nDims; ++k)
    scanf("%d", &corner0[k]);
  
  m_IF.printf("Enter other corner (%d coordinates): ", nDims);
  for (k = 0; k < nDims; ++k)
    scanf("%d", &corner1[k]);
  ...
  hilbert_box_vtx(nDims, sizeof(unsigned), 8*sizeof(unsigned),
		  1, cornerlo, work);
  m_IF.printf("Predicted lo corner: ");
  for (k = 0; k < nDims; ++k)
    m_IF.printf("%4u", cornerlo[k]);
  m_IF.printf("\n");
  ...
  hilbert_box_vtx(nDims, sizeof(unsigned), 8*sizeof(unsigned),
		  0, work, cornerhi);
  m_IF.printf("Predicted hi corner: ");
  for (k = 0; k < nDims; ++k)
    m_IF.printf("%4u", cornerhi[k]);
  m_IF.printf("\n");
  ...
  m_IF.printf("Result of sort\n");
  for (k = 0; k < (1<<nDims); k += (1 << nDims) - 1)
    {
      unsigned j;
      unsigned* eltk = &array[k][0];
      for (j = 0; j < nDims; ++j)
	m_IF.printf("%4u", eltk[j]);
      m_IF.printf("\n");
    }
  free((char*)array);
  return 0;
...
int main()
{
  ...
  m_IF.printf("Enter one point (%d coordinates): ", nDims);
  for (k = 0; k < nDims; ++k)
    scanf("%d", &point0[k]);
  
  m_IF.printf("Enter other point (%d coordinates, strictly greater): ", nDims);
  for (k = 0; k < nDims; ++k)
    {
      unsigned diff;
      scanf("%d", &point1[k]);
  ...
  hilbert_box_pt(nDims, sizeof(unsigned), 8*sizeof(unsigned),
		 1, pointlo, work);
  m_IF.printf("Predicted lo point: ");
  for (k = 0; k < nDims; ++k)
    m_IF.printf("%4u", pointlo[k]);
  m_IF.printf("\n");
  ...
  hilbert_box_pt(nDims, sizeof(unsigned), 8*sizeof(unsigned),
		 0, work, pointhi);
  m_IF.printf("Predicted hi point: ");
  for (k = 0; k < nDims; ++k)
    m_IF.printf("%4u", pointhi[k]);
  m_IF.printf("\n");
  ...
  if (array == 0)
    {
      fm_IF.printf(stderr, "Out of memory.\n");
      exit(-1);
    }
  nextItem = 0;
...
int main()
{
  ...
  m_IF.printf("Enter 1st box corner: ");
  for (i = 0; i < nDims; ++i)
    scanf("%u", &c1[i]);
  m_IF.printf("Enter 2nd box corner: ");
  for (i = 0; i < nDims; ++i)
    scanf("%u", &c2[i]);
  m_IF.printf("Enter point: ");
  for (i = 0; i < nDims; ++i)
    scanf("%u", &pt[i]);
  m_IF.printf("Find prev?: ");
  scanf("%d", &findPrev);
...
***   http://and-what-happened.blogspot.de/2011/08/fast-2d-and-3d-hilbert-curves-and.html
************************************************************************************************/


unsigned long MortonToHilbert3D( const unsigned long morton, const unsigned long bits )
{
	unsigned long hilbert = morton;
	if( bits > 1 ) {
		unsigned long block = ( ( bits * 3 ) - 3 );
		unsigned long hcode = ( ( hilbert >> block ) & 7 );
		unsigned long mcode, shift, signs;
		shift = signs = 0;
		while( block ) {
			block -= 3;
			hcode <<= 2;
...
unsigned long HilbertToMorton3D( const unsigned long hilbert, const unsigned long bits )
{
	unsigned long morton = hilbert;
	morton ^= ( ( morton & 0x92492492 ) >> 1 );
	morton ^= ( ( morton >> 1 ) & 0x92492492 );
	if( bits > 1 ) {
		unsigned long block = ( ( bits * 3 ) - 3 );
		unsigned long hcode = ( ( morton >> block ) & 7 );
		unsigned long mcode, shift, signs;
		shift = signs = 0;
		while( block ) {
			block -= 3;
...
unsigned long Morton_3D_Encode_10bit( unsigned long index1, unsigned long index2, unsigned long index3 )
...
void Morton_3D_Decode_10bit( const unsigned long morton, unsigned long &index1, unsigned long &index2, unsigned long &index3 )
{ // unpack 3 10-bit indices from a 30-bit Morton code
	unsigned long value1 = morton;
	unsigned long value2 = ( value1 >> 1 );
	unsigned long value3 = ( value1 >> 2 );
	value1 &= 0x09249249;
	value2 &= 0x09249249;
	value3 &= 0x09249249;
	value1 |= ( value1 >> 2 );
...
void hilbert2_3d_i2c(unsigned long index, unsigned long &ix, unsigned long &iy, unsigned long &iz) {
...
unsigned long hilbert2_3d__c2i(unsigned long ix, unsigned long iy, unsigned long iz) {
