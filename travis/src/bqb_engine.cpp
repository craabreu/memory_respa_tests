...
void CBQBEngine::ExportCellInfo(CBQBCellInfo *ci, CBQBBitSet *bs) {
...
bool CBQBEngine::ImportCellInfo(CBQBCellInfo *ci, CBQBBitSet *bs) {
...
//				m_IF.printf("1B) %3d Sanitized rounding of %f.\n",zi,tpred[zi]*pow10(asigni));
	...
	if (m_IF.IsPL(BQB_PL_VERBOSE))
		m_IF.printf("*** AtomsToIntegerArray finished, %lu symbols written ***\n\n",outp.size()-i);
}
	...
	for (z=0;z<zt;z++) {
		...
		for (z2=0;z2<rem;z2++) {
			...
			if (as->m_oaAtoms[z*5000+z2]->m_sLabel.length() > 127) {
				m_IF.eprintf("CBQBEngine::ExportAtomLabels(): Error: Labels longer than 127 characters are not supported (%ld).\n",
